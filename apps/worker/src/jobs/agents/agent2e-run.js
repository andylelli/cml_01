...
import { executeAgentWithRetry, appendRetryFeedback, appendRetryFeedbackOptional, } from "./shared.js";
export async function runAgent2e(ctx) {
    ctx.reportProgress("background-context", "Generating background context...", 25);
    const setting = ctx.setting;
    const cast = ctx.cast;
    let backgroundContextResult;
    if (ctx.enableScoring && ctx.scoreAggregator && ctx.retryManager && ctx.scoringLogger) {
        const { result, duration, cost } = await executeAgentWithRetry("agent2e_background_context", "Background Context", async (retryFeedback) => {
    ...
    if (!backgroundContextValidation.valid) {
        backgroundContextValidation.errors.forEach((error) => ctx.errors.push(`Background context schema failure: ${error}`));
        throw new Error("Background context artifact failed schema validation");
    }
    backgroundContextValidation.warnings.forEach((warning) => ctx.warnings.push(`Background context schema warning: ${warning}`));
    ctx.reportProgress("background-context", "Background context generated", 28);
}
//# sourceMappingURL=agent2e-run.js.map