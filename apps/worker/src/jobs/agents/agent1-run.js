...
import { executeAgentWithRetry, appendRetryFeedbackOptional, } from "./shared.js";
export async function runAgent1(ctx) {
    ...
    if (ctx.enableScoring && ctx.scoreAggregator && ctx.retryManager && ctx.scoringLogger) {
        const { result, duration, cost } = await executeAgentWithRetry("agent1_setting", "Setting Refinement", async (retryFeedback) => {
    ...
    if (ctx.setting.setting.realism.anachronisms.length > 0 ||
        ctx.setting.setting.realism.implausibilities.length > 0) {
        ctx.errors.push(`Agent 1: Setting realism issues detected (anachronisms=${ctx.setting.setting.realism.anachronisms.length}, implausibilities=${ctx.setting.setting.realism.implausibilities.length})`);
...
//# sourceMappingURL=agent1-run.js.map