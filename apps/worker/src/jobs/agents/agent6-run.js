 ...
 */
import { auditFairPlay, extractClues, blindReaderSimulation, buildCMLPrompt, reviseCml, } from "@cml/prompts-llm";
import { getGenerationParams } from "@cml/story-validation";
import { applyClueGuardrails, } from "./shared.js";
function classifyFairPlayFailure(coverageResult, fairPlayAudit, cml) {
    const caseBlock = cml?.CASE ?? cml;
    const steps = caseBlock?.inference_path?.steps ?? [];
    const abstractSteps = steps.filter((s) => {
    ...
    const criticalRules = new Set((fairPlayAudit?.violations ?? [])
        .filter((v) => v.severity === "critical")
        .map((v) => String(v.rule ?? "").toLowerCase().trim()));
    if ((criticalRules.has("logical deducibility") || criticalRules.has("no withholding")) &&
...
// runAgent6
// ============================================================================
export async function runAgent6(ctx) {
    ...
    while (fairPlayAttempt < fairPlayConfig.retries.max_fair_play_attempts) {
        ...
        if (fairPlayAttempt < fairPlayConfig.retries.max_fair_play_attempts) {
            ctx.warnings.push(`Agent 6: Fair play audit ${fairPlayAudit.overallStatus}; regenerating clues to address feedback (attempt ${fairPlayAttempt + 1} of ${fairPlayConfig.retries.max_fair_play_attempts})`);
    ...
    let hasCriticalFairPlayFailure = fairPlayAudit.violations.some((v) => v.severity === "critical" || criticalFairPlayRules.has(v.rule));
    if (fairPlayAudit.overallStatus === "fail") {
        if (hasCriticalFairPlayFailure) {
            ctx.warnings.push("Agent 6: Fair play audit failed on critical rules after clue regeneration; continuing with warnings");
            fairPlayAudit.violations.forEach((v) => ctx.warnings.push(`  - [${v.severity}] ${v.rule}: ${v.description}`));
        }
        else {
            ctx.warnings.push("Agent 6: Fair play audit FAILED after clue regeneration");
            fairPlayAudit.violations.forEach((v) => ctx.warnings.push(`  - [${v.severity}] ${v.description}`));
        }
    }
    else if (fairPlayAudit.overallStatus === "needs-revision") {
        if (hasCriticalFairPlayFailure) {
            ctx.warnings.push("Agent 6: Fair play needs revision with critical issues; continuing with warnings");
            fairPlayAudit.violations.forEach((v) => ctx.warnings.push(`  - [${v.severity}] ${v.rule}: ${v.description}`));
        }
        else {
    ...
    const recordFairPlayScore = async () => {
        if (!ctx.enableScoring || !ctx.scoreAggregator || !fairPlayAudit)
            return;
        const fpStatus = fairPlayAudit.overallStatus;
        const fpValidation = fpStatus === "pass" ? 100 : fpStatus === "needs-revision" ? 70 : 45;
        ctx.scoreAggregator.upsertPhaseScore("agent6_fairplay", "Fair-play Audit", {
            ...
            total: fpValidation,
            grade: (fpValidation >= 90 ? "A" : fpValidation >= 80 ? "B" : fpValidation >= 70 ? "C" : fpValidation >= 60 ? "D" : "F"),
        ...
        }, ctx.agentDurations["agent6_fairplay"] ?? 0, ctx.agentCosts["agent6_fairplay"] ?? 0);
        try {
    ...
    const caseBlockForBlind = ctx.cml?.CASE ?? ctx.cml;
    const castNamesForBlind = (caseBlockForBlind?.cast ?? []).map((c) => c.name).filter(Boolean);
    ...
    if (castNamesForBlind.length > 0 && falseAssumptionStatement && actualCulpritName) {
        ...
        const blindResult = await blindReaderSimulation(ctx.client, ctx.clues, falseAssumptionStatement, castNamesForBlind, { runId: ctx.runId, projectId: ctx.projectId || "" });
        ...
        if (readerGotItRight && (blindResult.confidenceLevel === "certain" || blindResult.confidenceLevel === "likely")) {
            ...
            ctx.warnings.push(`Blind reader simulation: reader suspected "${blindResult.suspectedCulprit}" ` +
            ...
            if (blindResult.confidenceLevel === "impossible" || !readerGotItRight) {
                ctx.warnings.push("CRITICAL: Blind reader cannot identify culprit. " +
                ...
                hasCriticalFairPlayFailure = fairPlayAudit.violations.some((v) => v.severity === "critical" || criticalFairPlayRules.has(v.rule));
                await recordFairPlayScore();
            }
        }
    }
    ...
    if (fairPlayAudit.overallStatus === "fail" && hasCriticalFairPlayFailure) {
        ...
        if ((failureClass === "inference_path_abstract" || failureClass === "constraint_space_insufficient") &&
            ...
            ctx.warnings.push(`Fair play failure classified as "${failureClass}" — retrying CML generation (Agent 4) ` +
            ...
            const cmlYaml = typeof ctx.cml === "string" ? ctx.cml : JSON.stringify(ctx.cml, null, 2);
            ...
            const revisedSteps = (revisedResult.cml?.CASE ?? revisedResult.cml)?.inference_path?.steps ?? [];
            if (revisedSteps.length >= 3) {
                ctx.cml = revisedResult.cml;
        ...
        else if (failureClass === "clue_only" &&
            fairPlayRetryCost <= MAX_FAIR_PLAY_RETRY_COST &&
            fairPlayConfig.retries.max_total_attempts_with_targeted_regen >= 3) {
            ctx.warnings.push("Fair play failure classified as \"clue_only\" — CML structure is sound; " +
        ...
        if (fairPlayAudit.overallStatus === "fail") {
            ...
            if (criticalViolations) {
                ctx.warnings.push(`Fair play: critical failures persist after all retries: ${criticalViolations}`);
            }
        }
        if (fairPlayRetryCost > MAX_FAIR_PLAY_RETRY_COST) {
            ctx.warnings.push(`Fair play retry cost limit reached ($${fairPlayRetryCost.toFixed(3)}) — proceeding with best result`);
        }
    }
    ctx.fairPlayAudit = fairPlayAudit;
    ctx.hasCriticalFairPlayFailure = hasCriticalFairPlayFailure;
}
//# sourceMappingURL=agent6-run.js.map