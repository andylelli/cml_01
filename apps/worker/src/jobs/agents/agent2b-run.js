...
export async function runAgent2b(ctx) {
    ...
    if (ctx.enableScoring && ctx.scoreAggregator && ctx.retryManager && ctx.scoringLogger) {
        const { result, duration, cost } = await executeAgentWithRetry("agent2b_profiles", "Character Profiles", async (retryFeedback) => {
    ...
    ctx.reportProgress("profiles", `Character profiles generated (${ctx.characterProfiles.profiles.length})`, 89);
}
//# sourceMappingURL=agent2b-run.js.map