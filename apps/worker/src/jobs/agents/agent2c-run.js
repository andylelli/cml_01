...
import { appendRetryFeedback, executeAgentWithRetry, } from "./shared.js";
import { adaptLocationsForScoring } from "../scoring-adapters/index.js";
export async function runAgent2c(ctx) {
    ...
    if (ctx.enableScoring && ctx.scoreAggregator && ctx.retryManager && ctx.scoringLogger) {
        const { result, duration, cost } = await executeAgentWithRetry("agent2c_location_profiles", "Location Profiles", async (retryFeedback) => {
    ...
    ctx.reportProgress("location-profiles", `Location profiles generated (${ctx.locationProfiles.keyLocations.length} locations)`, 89);
}
//# sourceMappingURL=agent2c-run.js.map