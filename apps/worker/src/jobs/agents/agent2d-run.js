...
export async function runAgent2d(ctx) {
    ...
    if (ctx.enableScoring && ctx.scoreAggregator && ctx.retryManager && ctx.scoringLogger) {
        const { result, duration, cost } = await executeAgentWithRetry("agent2d_temporal_context", "Temporal Context", async (retryFeedback) => {
    ...
    ctx.reportProgress("temporal-context", `Temporal context generated (${ctx.temporalContext.specificDate.month} ${ctx.temporalContext.specificDate.year})`, 91);
}
//# sourceMappingURL=agent2d-run.js.map