...
import { executeAgentWithRetry, appendRetryFeedback, mergeHardLogicDirectives, } from "./shared.js";
export async function runAgent3b(ctx) {
    ...
    if (ctx.enableScoring && ctx.scoreAggregator && ctx.retryManager && ctx.scoringLogger) {
        const { result, duration, cost } = await executeAgentWithRetry("agent3b_hard_logic_devices", "Hard Logic Devices", async (retryFeedback) => {
    ...
    ctx.hardLogicDirectives = mergeHardLogicDirectives(ctx.initialHardLogicDirectives, ctx.hardLogicDevices.devices);
    ctx.reportProgress("hard_logic_devices", `Generated ${ctx.hardLogicDevices.devices.length} novel hard-logic devices`, 31);
}
//# sourceMappingURL=agent3b-run.js.map