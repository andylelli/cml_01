 ...
 */
function buildCastNameMap(castMembers) {
    const map = new Map();
    ...
    for (const member of castMembers) {
        ...
        for (const part of lower.split(/\s+/)) {
            if (part.length > 2 && (tokenCounts.get(part) ?? 0) === 1)
                map.set(part, full);
        }
    }
    return map;
 ...
 */
function resolveCharacterName(token, nameMap) {
    const key = token.toLowerCase().replace(/\s+/g, '_');
    return nameMap.get(key)
        ?? nameMap.get(token.toLowerCase())
        ?? token;
}
export function adaptNarrativeForScoring(narrative, castMembers, clueList) {
    const nameMap = castMembers ? buildCastNameMap(castMembers) : new Map();
    ...
    const chapters = (narrative.acts || []).flatMap((act, actIndex) => {
        return (act.scenes || []).map((scene, sceneIndex) => {
            ...
            const characters_present = (scene.characters || []).map(token => resolveCharacterName(token, nameMap));
            return {
...
//# sourceMappingURL=agent7-scoring-adapter.js.map