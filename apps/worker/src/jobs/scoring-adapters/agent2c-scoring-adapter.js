export function adaptLocationsForScoring(locResult) {
    ...
    const sharedMood = locResult.atmosphere?.mood
        ...
        || '';
    const location_profiles = keyLocations.map(loc => {
        ...
        const taste = smells.length > 0
            ? `The air carries a faint taste of ${smells.slice(0, 2).join(' and ')}${smells.length > 2 ? `, with hints of ${smells[2]}` : ''}, lingering on the palate.`
        ...
        const geoParts = [
            ...
            `within ${primary.name}`,
            primary.place
                ? `in ${primary.place}${primary.country ? `, ${primary.country}` : ''}`
                : primary.country || '',
            `— ${loc.purpose}`,
        ].filter(Boolean);
...
//# sourceMappingURL=agent2c-scoring-adapter.js.map