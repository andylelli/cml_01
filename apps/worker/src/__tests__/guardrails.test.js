...
const checkInferencePathCoverage = (cml, clues) => {
    const issues = [];
...
const checkContradictionPairs = (cml, clues) => {
    const issues = [];
    const caseBlock = cml?.CASE ?? cml;
    const steps = caseBlock?.inference_path?.steps ?? [];
    for (let i = 0; i < steps.length; i++) {
...
const checkFalseAssumptionContradiction = (cml, clues) => {
    const issues = [];
    const caseBlock = cml?.CASE ?? cml;
...
const checkDiscriminatingTestReachability = (cml, clues) => {
    const issues = [];
    const caseBlock = cml?.CASE ?? cml;
    ...
    const relevantClues = clues.clues.filter(c => {
        ...
        const testWords = designText.split(/\s+/).filter((w) => w.length > 4);
...
const checkSuspectElimination = (cml, clues) => {
    const issues = [];
    const caseBlock = cml?.CASE ?? cml;
    const cast = Array.isArray(caseBlock?.cast) ? caseBlock.cast : [];
    const culprits = caseBlock?.culpability?.culprits ?? [];
    const suspects = cast.filter((c) => c.culprit_eligibility === "eligible" && !culprits.includes(c.name));
...
const makeClueSet = (clueOverrides = []) => ({
...
describe("WP6A: classifyFairPlayFailure", () => {
    const classifyFairPlayFailure = (coverageResult, _fairPlayAudit, cml) => {
        const caseBlock = cml?.CASE ?? cml;
        const steps = caseBlock?.inference_path?.steps ?? [];
