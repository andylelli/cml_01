...
const toNumericValue = (value) => typeof value === "number" && Number.isFinite(value) ? value : null;
const toFailedChapterRanges = (value) => {
    ...
    return value
        ...
        if (!entry || typeof entry !== "object")
            return null;
        const start = toNumericValue(entry.start);
        const end = toNumericValue(entry.end);
...
const toRequiredClueIdsByChapter = (value) => {
    if (!Array.isArray(value))
        return [];
    return value
        .map((entry) => {
        if (!entry || typeof entry !== "object")
            return null;
        const chapter = toNumericValue(entry.chapter);
        if (chapter === null)
            return null;
        const rawIds = entry.required_clue_ids;
        const ids = Array.isArray(rawIds)
            ? rawIds.filter((candidate) => typeof candidate === "string")
            : [];
        return { chapter, required_clue_ids: ids };
    })
        .filter((entry) => entry !== null);
};
const toNormalizedRequiredClueIdsByChapter = (value) => {
    if (!Array.isArray(value))
        return [];
    return value
        .map((entry) => {
        ...
        if (chapter === null)
            return null;
        const rawIds = entry.normalized_required_clue_ids;
        const ids = Array.isArray(rawIds)
            ? rawIds.filter((candidate) => typeof candidate === "string")
            : [];
        return { chapter, normalized_required_clue_ids: ids };
    })
        .filter((entry) => entry !== null);
};
const deriveProsePresentation = (reportLike) => {
    const diagnostics = reportLike.diagnostics;
    if (!Array.isArray(diagnostics))
        return null;
    const postGeneration = diagnostics.find((diagnostic) => {
        if (!diagnostic || typeof diagnostic !== "object")
            return false;
        const candidate = diagnostic;
        return (candidate.key === "agent9_prose_post_generation_summary"
            || candidate.diagnostic_type === "post_generation_summary");
    });
    if (!postGeneration || typeof postGeneration !== "object")
        return null;
    const details = postGeneration.details;
    if (!details || typeof details !== "object")
        return null;
    const detailsRecord = details;
    const reconciliation = detailsRecord.count_reconciliation;
    const reconciliationRecord = reconciliation && typeof reconciliation === "object"
        ? reconciliation
        : null;
    const expected = toNumericValue(detailsRecord.expected_chapters_total)
        ?? toNumericValue(detailsRecord.expected_chapter_count);
    const committed = toNumericValue(detailsRecord.chapters_generated_committed)
        ?? toNumericValue(detailsRecord.chapters_generated);
    const failed = toNumericValue(detailsRecord.chapters_failed_uncommitted);
    const rawRunState = reconciliationRecord?.run_state;
    const run_state = rawRunState === "in_progress"
        || rawRunState === "partial_generation"
        || rawRunState === "complete"
        ? rawRunState
        : "unknown";
    return {
        run_state,
        chapters: {
            expected,
            committed,
            failed_uncommitted: failed,
            failed_uncommitted_ranges: toFailedChapterRanges(reconciliationRecord?.failed_uncommitted_chapter_ranges),
        },
        words: {
            viability_hard_floor: toNumericValue(detailsRecord.chapter_words_hard_floor),
            quality_preferred_target: toNumericValue(detailsRecord.chapter_words_preferred_target),
            completeness_story_total: toNumericValue(detailsRecord.story_words_total),
            completeness_story_min: toNumericValue(detailsRecord.expected_story_min_words),
            completeness_story_max: toNumericValue(detailsRecord.expected_story_max_words),
        },
        clue_obligations: {
            required_ids_by_chapter: toRequiredClueIdsByChapter(detailsRecord.clue_obligation_required_ids_by_chapter),
            normalized_required_ids_by_chapter: toNormalizedRequiredClueIdsByChapter(detailsRecord.clue_obligation_normalized_required_ids_by_chapter),
        },
    };
};
const withProsePresentation = (reportLike) => {
    const prosePresentation = deriveProsePresentation(reportLike);
    if (!prosePresentation)
        return reportLike;
    return {
        ...reportLike,
        prose_presentation: prosePresentation,
    };
};
...
export const createServer = () => {
    ...
    app.get("/api/projects/:projectId/runs/:runId/report", async (req, res) => {
        ...
        const sendLatestValidFallback = async (staleReason) => {
            try {
                ...
                latestValidReports.sort((a, b) => new Date(String(b.generated_at ?? 0)).getTime() -
                    new Date(String(a.generated_at ?? 0)).getTime());
                if (latestValidReports.length > 0) {
                    const latest = withProsePresentation(latestValidReports[0]);
                    res.status(200).json({
                        ...latest,
                        stale: true,
        ...
        try {
            ...
            if (report.in_progress === true) {
                if (await isKnownActiveRun()) {
                    // Surface partial scoring snapshots during active runs so the UI can
                    // render quality metrics progressively instead of waiting for completion.
                    const reportWithPresentation = withProsePresentation(report);
                    res.status(200).json({
                        ...reportWithPresentation,
                        status: "in_progress",
                        projectId: req.params.projectId,
                        runId: req.params.runId,
                    });
            ...
            res.json(withProsePresentation(report));
        }
        catch {
            if (await isKnownActiveRun()) {
                res.status(202).json({
    ...
    app.get("/api/projects/:projectId/reports/history", async (req, res) => {
        try {
            ...
            const reportsWithPresentation = reports.map((report) => withProsePresentation(report));
            res.json({
                projectId: req.params.projectId,
                limit,
                count: reportsWithPresentation.length,
                reports: reportsWithPresentation,
            });
        }
        catch {
        ...
        try {
            ...
            for (const type of artifactTypes) {
                ...
                if (type === "quality_report" || type === "scoring_report") {
                    ...
                    if (latestReport) {
                        results.quality_report = withProsePresentation(latestReport);
                    }
                    continue;
                }
                if (type === "narrative_state_trace") {
                    const latestReport = await getLatestQualityReport();
                    if (latestReport) {
                        const nsdDiagnostic = latestReport.diagnostics?.find((diagnostic) => diagnostic.key === "agent9_prose_post_generation_summary"
                            || diagnostic.diagnostic_type === "post_generation_summary");
                        const nsdTrace = nsdDiagnostic?.details?.nsd_transfer_trace;
