...
async function main() {
    ...
    for (const project of projects) {
        ...
        if (existingSettingVal) {
            ...
            if (settingArtifact) {
                const settingRealism = settingArtifact.payload?.setting?.realism ?? {};
                const errors = [
        ...
        if (existingCastVal) {
            ...
            if (castArtifact) {
                const castDiversity = castArtifact.payload?.cast?.diversity ?? {};
                const errors = castDiversity.stereotypeCheck ?? [];
                const warnings = castDiversity.recommendations ?? [];
                const artifact = {
                    valid: errors.length === 0,
                    errors,
                    warnings,
...
//# sourceMappingURL=migrate-validation-artifacts.js.map