...
const normalizeProseCastOrThrow = (inputCast: unknown): CastDesign => {
    ...
    for (let attempt = 1; attempt <= maxBatchAttempts; attempt++) {
      try {
        ...
        for (let i = 0; i < proseBatch.chapters.length; i++) {
          ...
          if (attempt >= maxBatchAttempts && Array.isArray(chapter.paragraphs) && chapter.paragraphs.length < 3) {
            const before = chapter.paragraphs.length;
            const repaired = enforceMinimumParagraphStructure(chapter, 3);
