...
| `AGENT9_CLEARANCE_TRIM` | unset -> off | **PROBE NEXT RUN** | **A_75 §12, 2026-08-27. The most frequent defect in the pipeline had no repair.** `clearance_over_budget` fires on **17 of 23 archived runs (74%)** — ahead of `reveal_culprit_not_named` (61%) and `unaccounted_time` (57%) — and `story-geometry/src/codes.ts` says so itself: *"too MANY clearances — a negative constraint, and the only negative pass that exists (aftermath_repeat) removes repetition rather than trimming a register"*. BOTH external readers of the 2026-08-27 matched pair complained about it unprompted and both tied it to the ENDING — *"it slows the ending"*, *"still unnecessary after Chapter 8"*. `AGENT9_FOLD_SUSPECT_CLEARANCES` is already ON and is only a prompt steer; the model re-argues anyway. On: redundant clearance sentences are removed down to the geometry budget, BEFORE acceptance runs, so the gate counts the trimmed text — a repair the gate cannot see is a repair nobody can verify. **Deletion is safe by construction**: it only ever removes a clearance whose suspect was already cleared in an EARLIER chapter, so a `suspect_closure_missing` run-killer cannot be introduced, and it trims only DOWN TO the budget rather than to zero (a callback is allowed; the register is what readers object to). The recognisers are a documented MIRROR of `story-geometry/accept.ts` — prompts-llm must not depend on story-geometry — and a test reads that file and asserts the regex literals are byte-identical, so a repair can never trim a different set than the gate counts. Every removal is logged. Probe: one run; `clearance_over_budget` should fall from 74% baseline, and read `ending` (+1 headroom) and `pacing` (never above 8 in 42 reads). Falsification: if the code clears and neither mark moves, the register was not what the readers were marking. |
...
| `AGENT9_REGEN_CONVERGENCE_STOP` | unset -> off | **PROBE NEXT RUN** | **Agent 9 review PASS 12, 2026-08-27. The regen loop had no ceiling.** `maxAttemptsPerDefect` (2) was the only bound and the DEFECT COUNT is unbounded, so total calls are 2 x defects with no limit anywhere. MEASURED from the arm-A LLM log: regen was the single largest consumer of the run — 33 calls, 40% of wall-clock, MORE than chapter generation itself (37%) — and it was not spread: **ch3 took 20 of the 33** (six `missing_clue` x 2, three `clue_too_late` x 2, one scaffold x 2) and **every one failed**. The deterministic floor then fired and pasted the key-term bags anyway; the external reader's note on that same manuscript was *"the worst leakage is in Chapter 3"*. Those twelve calls could not have worked — the defects share a chapter and a cause, and a rewrite that cannot surface clue 1 will not surface clue 5. On: after N consecutive failures of ONE defect kind in one chapter (`AGENT9_REGEN_CONVERGENCE_LIMIT`, default 3) the remaining defects of that kind are not attempted. **The stop is on consecutive failures, not a call count** — a chapter that is actually being repaired resets the counter and keeps its full budget, and kinds are counted separately so one hopeless kind cannot starve another. **Nothing is silently dropped**: skipped defects go to `unresolved` UNTOUCHED, which is exactly where a failed repair would have put them, so the deterministic floor's input is unchanged and no obligation goes unmet. Probe: one run; compare regen call count against the 33 baseline and confirm the manuscript's obligation coverage is identical. Falsification: if coverage drops, the skipped repairs were succeeding sometimes and the limit is too tight. |
