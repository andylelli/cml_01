...
| **X43** | ~~The voice fragments are copied, not matched~~ — **BUILT 2026-08-17**: Agent 6.5's `voiceFragments` reached Agent 9 under *"match this register and rhythm"* with no anti-verbatim guard, the ONE voice input in that prompt without one. 28–50% returned as verbatim ≥5-word spans, tracking the reader's dialogue mark with no overlap between the 7s and the 6s. Same guard as the `signatureTic` two lines above; `probe:voice-fragment-leak` is the instrument. **PROBED 2026-08-17 and the cure FAILED: 6/18 = 33%, identical to the 86, with the guard verified present in every chapter that leaked** — and all six leaks fall in ch1–5, the only chapters X47 leaves the fragments in. Next move is to withhold or paraphrase the sample, not to firm up the sentence | free | ◑ | [REVIEW_10 §4](REVIEW_10.md) · [§10.3](REVIEW_10.md) |
...
| **X51** | **The locked-fact registry is device-scoped, so the weapon and the alibi locations are pinned nowhere** — `agent3b-run.ts` populates it from `hardLogicDevices.devices[0].lockedFacts` alone; the device is a timing trick, so all three facts are times. The pin WORKS (*"a quarter to three"* is verbatim across five chapters) and the consistency gate is value-agnostic. Unpinned: the murder weapon (X48's second half) and every suspect's alibi location — Hale gives **four** across five statements (patrol / rounds+lobby / lounge / smoking room), Beatrice gives front desk (ch3), kitchen (ch4) and **both in one ch9 sentence**. Closes two of the reader's six items with one change **BUILT 2026-08-18** — `buildCaseScopedLockedFacts` in `@cml/cml`, wired at the END of `runAgent3` (not Agent 3b: the registry is built before Agent 3, so CASE.death_method and alibi_window do not exist yet, and the novelty retry can still replace ctx.cml after). On the real 08-17 case: `murder_weapon="a heavy bronze statuette"`, plus the smoking room / front desk / garden lounge. Detective, victim and culprit excluded. **Every value refused if it parses as a clock time or duration** - locking an alibi WINDOW rather than its PLACE would have switched off X38, which fires only on 2 clocks + 1 duration. 15 tests | free | :white_check_mark: | [REVIEW_11 §8.1](REVIEW_11.md) |
| **X52** | **Agent 9 authors the decisive trace in the reveal chapter, and plant/payoff cannot see it** — `navy wool` appears in NO prompt before `Agent9-ProseGenerator-Ch8`; every later hit is the ch8 draft echoing through the continuity tail. It is in no clue set, outline or contract, so nothing upstream could plant it. `clincher_not_planted` stayed silent because it checks the CONTRACT's trace (the escapement scoring, correctly planted in ch1) rather than the one the manuscript uses. NSD saw the shadow and filed it advisory: *"1 clue(s) visible in prose but absent from NSD outline plan"*. Reader problem #3: *"appears for the first time in the reveal"* — which nothing could have prevented. Cheap fix: promote the advisory to a finding **BUILT 2026-08-18** — diagnosis CORRECTED on build, and the correction is the finding. The navy wool was NOT invented by Agent 9: it is `clue_late_optional_slot_1` ("A torn piece of Hugo Vane's cuff..."), authored by Agent 5 and present in ch8's prompt. The real cause is that `applyPlantBeforeReveal` filters on `criticality === "essential"` and this clue is **optional**, so the one clue putting the culprit at the scene was the one the planting pass ignored. `applyDecisiveTracePlant` plants culprit-implicating PHYSICAL clues regardless of criticality, at the latest scene >=2 before the reveal and never in the opening (there it is the answer, not a plant). 8 tests | free | :white_check_mark: | [REVIEW_11 §8.2](REVIEW_11.md) |
| **X53** | **The voice-fragment probe measures span length, not conspicuousness** — the leak rate was 33% on the 08-06 read (dialogue 7, *"catchphrases still awkward"*) and 33% on the 08-17 read (dialogue **8**, no catchphrase complaint). Same count, different content: *"that's the way the biscuit crumbles"* vs *"I have nothing to hide"*. The five-word floor was right for a provenance check and is the wrong instrument for the complaint it tracks. Needs a distinctiveness term (idiom list, or corpus frequency across the 33 manuscripts) **BUILT 2026-08-18** — two terms, both measured. LENGTH: share of leaked spans at 7+ words is **0% / 17% / 28% / 39% / 44%** against dialogue **8** / 7 / 7 / 6 / 6 - monotone, no overlap between mark bands, and the one read with no catchphrase complaint separates to ZERO on a manuscript whose old rate was identical to the 86's. RARITY (demotion only): over 184 corpus books "i have nothing to hide" appears in **51**, "well that s the way the biscuit crumbles" in **1**; it bottoms out at 1 for real signatures and merely-uncommon phrasings alike, so it can prove a span ordinary, never prove one distinctive | free | :white_check_mark: | [REVIEW_11 §2](REVIEW_11.md) |
| **X54** | **The disclosure attribution read only one English word order** — `ADMISSION_ATTRIBUTION` (X34, shipped 08-14) matched VERB-then-NAME (`said Beatrice Quill`) and not NAME-then-VERB (`Beatrice Quill said`), and only the covered order had a test. Measured: the same paragraph, same confession by the same wrong person, returns `unmet` in one order and **`met`** in the other — a second character's admission satisfying the culprit's disclosure obligation, the FALSE CERTIFICATION class §10.1 exists to prevent, which then hands the judge `noResolution = false` where the detector outranks it. **FIXED 2026-08-18** as one regex with two alternatives so leftmost-match returns the attribution NEAREST the admission. 6 tests, both orders, every attribution verb | free | :white_check_mark: | [REVIEW_11 §13.1](REVIEW_11.md) |
| **X55** | **The case's own murder device read as the SEASON** — on run `mystery-1787090659145` the case's evidence list declares *"Broken spring fragment found near rooftop pulley"*, and the chapter validator reported `month/season contradiction (september vs spring)` on ch2/3/4/8, twice each: **8 of that run's 11 retry calls**. Agent 2d's data was CORRECT (`month: September`, `season: fall`). `buildSpringMechanicalRe` exists for exactly this and could not reach it — built from mechanism NOUNS and winding ACTIONS, it strips "mainspring"/"clock spring" but not "broken spring fragment", not "a loose wire and a broken spring", and not the bare "the spring" English uses after first mention. **FIXED 2026-08-19**: `caseNamesMechanicalSpring` reads the case's physical traces / access objects / mechanism / death method; when the case names a spring, the seasonal test requires a marker a clock part cannot carry (`springtime`, `vernal`, or a measured seasonal collocation). Verified on the shipped manuscript: all four chapters clear, the other six unchanged, and a genuine "spring air" in September STILL flags. 12 tests | free | :white_check_mark: | [REVIEW_12 §3.1](REVIEW_12.md) · [§8.1](REVIEW_12.md) |
| **X56** | **Wordlist failures named a category, never the token** — *"chapter must not contain confession/arrest/solution language"* on a chapter with no confession, no arrest and no solution; the offending word was `exposed`, one of nineteen alternatives, in *"the mechanism inside the clock, partially exposed where the panel hung ajar"*. The model regenerated the whole chapter twice with no way to find what broke it. **FIXED 2026-08-19**: the discovery-opening, aftermath and season failures now quote the matched token with surrounding context and say what kind of edit resolves it. `analyzeTemporalConsistency` gained `seasonTriggers` so the caller has the span, not just the season name | free | :white_check_mark: | [REVIEW_12 §3.2](REVIEW_12.md) · [§8.2](REVIEW_12.md) |
| **X57** | **`RESOLUTION_RE` and the aftermath phrase list, narrowed on corpus evidence** — bare `expos(ed\|es)` occurs 322 times across 171 manuscripts (clock workings, exposed mechanisms, "the fragile order was exposed"); `surrendered` is mostly "dusk surrendered to the chill"; `guilty` is mostly "a guilty hand"; and `confess` caught the Golden-Age discourse marker *"Though I confess, I've seen clocks and men both falter"*. Each alternative now requires the construction carrying the resolution MEANING. **Checked at BOTH call sites** — this regex is the negative test for ch1 and the POSITIVE test for the reveal: over the last two chapters of all 171 manuscripts it now matches **169 where the old matched 165**, gaining `confession` (the noun). Separately, `for the first time` was REMOVED from the aftermath four: 47 occurrences in aftermath chapters, **not one about evidence** — it is the consequence-and-retrospect register the validator demands. 20 tests | free | :white_check_mark: | [REVIEW_12 §3.3](REVIEW_12.md) · [§8.3](REVIEW_12.md) |
| **X58** | ~~The retry ladder drops the rejected draft on the final attempt~~ — **NOT A DEFECT, and REVIEW_12 §4 was wrong to call the ladder undocumented.** `buildRetryPromptStrategy` is a considered policy with a computed `rationale`: a final attempt, a template failure or a multi-family failure all withhold the prior draft on the stated grounds that prior wording is a harmful anchor; only a single-family failure gets `surgical_patch` with the draft attached. The model is TOLD, in the prompt: *"Retry mode: FULL_REBUILD … Why this mode: final-attempt or hard mitigation triggered … Do not preserve sentence structure from the failed draft."* No code change made. One real residue: the code comment describes "attempts 2/3 include the previous response… attempt 4+ rebuild" while `maxBatchAttempts` is 3, so the intended two edit passes collapse to one — a comment/config mismatch, recorded not silently resolved | free | :white_check_mark: no-op | [REVIEW_12 §8.4](REVIEW_12.md) |
| **X59** | **The `shot` idiom had a THIRD word order, and it was the dangerous one** — X27 covered `shot a glance` (verb-noun) and X49b covered `the glance she shot` (relative clause). English also writes it DITRANSITIVELY: *"She shot Hugo Vane a look that was more tired than accusatory"* (ch2 of story_20260819-0147). That form puts a NAME between the verb and the noun, so the sentence pairs the culprit with a guilt verb — the exact conjunction `disclosingSentence` hunts for. It made `findManuscriptDisclosure` report the whole manuscript as disclosing at **chapter 2**, in a book whose reveal is chapter 10: a false certification, the class §10.1 exists to prevent. **FIXED 2026-08-19.** MEASURED: 25 sentences across 171 manuscripts match the ditransitive form, **all 25 are the idiom**, none covered by an existing branch; a real shooting has no article-plus-look-noun after the recipient. Backtest PASS, separation unchanged. Manuscript disclosure now reports ch10 | free | :white_check_mark: | [REVIEW_12 §9](REVIEW_12.md) |
| **X60** | **Agent 4 waives a defect; Agent 9 aborts on it — five paid agents apart** — on run `mystery-1787089592928` Agent 4 ran out of revision budget on `apparent_not_covered`, logged *"proceeding with 1 unresolved validation warning"*, and the run continued through Agents 5, 6, 6.5, 7 and 7.5. Agent 9's preflight re-ran the SAME `validateCml` and threw. **12.8 minutes and ~17 LLM calls after the defect was known and consciously waived, for no manuscript.** The graceful-degrade path CANNOT deliver a story for these codes: they are computed purely from `hidden_model.mechanism` plus the culprit's `alibi_window`, and nothing between Agent 3 and Agent 9 writes either (normalization only fills defaults) — so the late abort is deterministic, not bad luck. **FIXED 2026-08-19**: the degrade branch aborts on `apparent_not_covered`, `actual_covered` and `times_identical`, scoped to those three rather than to "validation invalid" because the general case cannot be proven unrepairable from there. Changes no run that would otherwise have shipped. 4 tests | free | :white_check_mark: | [REVIEW_12 §9](REVIEW_12.md) |
| **X61** | **The device's second clock is in the MECHANISM, and the parser could not read it either** — [REVIEW_13](REVIEW_13.md) §3 recorded X38 as *"intermittent: fired 08-17, not 08-19"* and §7 offered two clean runs as the test that would demote it. The 08-19 run is NOT clean: `apparent_time_of_death` *"a quarter to three"* and `actual_time_of_death` *"fourteen minutes past four"* are **89 minutes apart** while `disengagement_duration` declares **forty-five** — the third consecutive run with the same defect. X38 declined for two STACKED reasons, and reading the code found only the first: (1) the registry locked ONE clock, not two, so the `clocks.length === 2` shape gate is false; and (2) **`parseClockTime("fourteen minutes past four")` returned `null`**, because the minute vocabulary was a closed list of six values — so the case's own true time was unreadable to every temporal gate downstream while the false one read perfectly. The manuscript duly invented a third time, *"eight fifty-six"*, and nothing objected; `"fourteen minutes past four"` appears in it **zero times**. **FIXED 2026-08-19**: `parseClockTime` reads any minute count 1–59 (word, compound or digit) when the literal word *"minutes"* is present, and the X38 arithmetic falls back to the mechanism's two anchors when the registry locks only one clock. Wired already — `agent75-run.ts` passes both anchors — and reported as [X38] rather than [X39] now the code, not the stage, picks the label. The corpus sweep also found a defect the six-value list SHIPPED: `` matches after a hyphen, so *"forty-five minutes past one"* (7 occurrences) was read as **1:05**. 11 tests | free | :white_check_mark: | [§12.7](#127-x61--the-second-clock-and-the-parser-that-could-not-read-it) · [REVIEW_13 §3](REVIEW_13.md) |
| **X62** | **A trailing `` neuters a stem, and both outline-closure wordlists shipped with one** — `(...|eliminat|...)` cannot match *eliminate*, *eliminating* or *eliminated*: after `eliminat` comes a word character, so the closing boundary fails. It matched only the bare string `eliminat`, which no outline has written. `alibi` missed *alibis* and `clue` missed *clues* the same way. **MEASURED over the 32 archived Agent 7 outlines: the closure floor fired on 17 of 32 before, 8 of 32 after — nine outlines were told they had no suspect-closure coverage while their own scenes read *"Clearing the Others"* and *"verifying their alibis"*.** That false alarm is not free: `missing_suspect_closure_scene` drives an outline retry, a repair guardrail, AND a deterministic patch that appends clearance language to an Act 3 scene — and it is a **blocking abort** at Agent 9. So a wordlist that could not read the outline was manufacturing the duplicate clearances three external reads complained about. **FIXED 2026-08-19**, inflections only, no new vocabulary. This is why X32 measured 1 clearance scene on 08-19 instead of 2 | free | :white_check_mark: | [§12.8](#128-x32--x62--the-clearance-floor-with-no-ceiling-and-the-wordlists-that-could-not-read-the-outline) · [REVIEW_13 §8.2](REVIEW_13.md) |
| **X63** | **The per-act behaviour contract is role-blind, and its own comment claimed otherwise** — `assembleCharacterBundle` documents `permittedBehavioursByAct` as *"derived from motive seed + role"* and derives it from motive seed alone: `CharacterProfilesResult` carries **no role field**, so role was never in scope. It is the ONLY per-character behavioural steering that reaches Agent 9 (`prompt-blocks.ts` prints it verbatim as *"Act N behaviour contract"*). On 08-19 that meant **the DETECTIVE was told** *"May show unease, evasion, or mild defensiveness when questioned. One behavioural tell is permitted"* — the detective is not a suspect, and a behavioural tell is the currency X49's GUILT_MARKER sweep exists to police — and **the VICTIM, dead since scene 1, was given a live three-act contract** ending in *"confrontation, confession, or vindication"*. `act3` read nothing from the profile at all, so it was **byte-identical for every character in every run**. **FIXED 2026-08-19**: detective and victim get their own contracts; `act3` now carries the character's internal conflict. **Culprit and innocent suspects deliberately still share one contract** — a distinct allowance for the culprit is a behavioural tell the innocents lack, i.e. early disclosure by construction (X59). 7 tests. Separately, Agent 7 **drops** the per-character half when copying the contract into the outline (all 10 scenes, every character); harmless today because Agent 9 reads the bundle, not the outline | free | :white_check_mark: | [§12.9](#129-x63--a-behaviour-contract-that-did-not-know-who-it-was-writing-for) · [REVIEW_13 §8.5](REVIEW_13.md) |
| **X64** | **The injector's "rotation" could not rotate, so A_62's fix changed nothing observable** — `pickVariant` seeds on the locked fact's VALUE, which is invariant across the manuscript by definition. Same seed every time → same variant every time → `MAX_INJECTIONS_PER_FACT = 2` delivers two VERBATIM duplicates. Shipped in story_20260819-0147: *"It had taken forty-five minutes in all."* **×2** and the wire sentence **×2** — the injector's entire contribution to that manuscript, all of it duplicated. The A_62 comment declares the arms race over (*"no FIXED string"*); this is the third time the code lost it. The regression test pinned *"different VALUES pick different variants"* — true and irrelevant, since one manuscript has one value per fact. **FIXED 2026-08-19**: the variant is chosen by the OCCURRENCE ORDINAL, so the two injections of a fact are provably different rather than a coin flip; the value still picks the starting variant, so reproducibility holds. 10 tests | free | :white_check_mark: | [REVIEW_14 §10](REVIEW_14.md) |
...
### 12.7 X61 — the second clock, and the parser that could not read it

REVIEW_13 §7 named the test that would demote X38: *"the next two runs both produce coherent device
arithmetic"*. The 08-19 run was believed to be the first of those two. It is not.

```
the case, from logs/agent9-checkpoint-mystery-1787090659145.json + the Agent 3 prompt log
  locked   false_display_time         "a quarter to three"        2:45
  locked   disengagement_duration     "forty-five minutes"          45
  locked   wire_length                "twenty-five feet"             —
  mech     apparent_time_of_death     "a quarter to three"        2:45
  mech     actual_time_of_death       "fourteen minutes past four" 4:14

  4:14 − 2:45 = 89.  The device declares 45.
```

Three consecutive runs now carry this defect — 08-15 (10 against 14), 08-17 (25 against 20), 08-19
(89 against 45). It is not intermittent. **What was intermittent is the detector.**

X38 declined twice over, and only the first reason is visible in the source:

1. **The shape gate.** `clocks.length === 2 && durations.length === 1`. The 08-19 registry locks one
   clock and one duration, because the OTHER clock was never locked — it lives in
   `hidden_model.mechanism`. The gate is right to refuse a pairing it cannot infer from three loose
   clocks; it was wrong to refuse one the mechanism *names*.

2. **The parser.** `parseClockTime("fourteen minutes past four")` returned `null`. `MINUTE_WORDS` was
   a closed list of six values — quarter, half, ten, five, twenty, twenty-five — and *fourteen* is not
   among them. The OFFSET parser could read "fourteen minutes" perfectly; only the POSITION parser
   could not. So the case's false time parsed and its true time did not, everywhere, in every gate
   built on this function.

The second is why the manuscript reads as it does. The true time never entered the registry, so it
never reached the page — `"fourteen minutes past four"` appears **zero times** in
`story_20260819-0147` — and the prose invented its own true time, *"eight fifty-six"*, against alibis
running eight to nine. The book therefore states a clock frozen at 2:45, a real time of 8:56, and a
45-minute mechanism: **three times, no two of which agree**, and no check said a word.

**Found by probe, not by reading**, and that mattered three times over. Reading the code found the
shape gate and would have shipped a fallback that still could not parse its own input. Running it
found the null. Running it again found that the fix's first regex matched `"read forty minutes past
seven"` as garbage and returned null, and that `` matches after a hyphen — which is how
*"forty-five minutes past one"* has been read as **1:05** for as long as the six-value list has
existed, on 7 occurrences in the corpus.

**Not escalated to an abort.** X38 has no repair path by design — locked facts are contractual — so it
stays a warning at Agent 7.5, which is the first stage that can see both spines. What changes is that
it now *fires*.

### 12.8 X32 / X62 — the clearance floor with no ceiling, and the wordlists that could not read the outline

REVIEW_13 §5 put `AGENT9_FOLD_SUSPECT_CLEARANCES` first: pacing's only named defect, in three reads,
one flag flip plus one run. Reading the code before spending the run found that **the flag could not
have helped**, and then found why the defect exists.

**The flag's guardrail hangs off the FLOOR.** `buildOutlineRepairGuardrails` emitted the fold
instruction only when `missing_suspect_closure_scene` fired — the issue raised when an outline has NO
closure language anywhere. An outline that repeats clearances satisfies that check by definition, so
the trigger is unreachable in exactly the case the instruction addresses. Folding cannot repair an
absence. Across the whole prompt log the fold text was emitted **zero** times; the dedicated-scene
text fired 3 times, all on 07-31 and 08-02, none on 08-17 or 08-19.

**The floor has no ceiling.** Check 2 joins every scene into one string:

```js
const allSceneText = allScenes.map(...).join(" ");            // ALL scenes, one string
const hasAnyClosure = ELIMINATION.test(allSceneText) && EVIDENCE.test(allSceneText);
if (!hasAnyClosure) { issues.push({ type: "missing_suspect_closure_scene" }) }
```

Three clearance scenes satisfy it exactly as one does. Check 1, ten lines above, tests
**per scene**. The inconsistency is the bug.

**And the wordlists could not read the outline.** `(...|eliminat|...)` cannot match "eliminate" —
the closing boundary fails against the following word character — so the stem matched nothing at all.
`alibi` missed "alibis"; `clue` missed "clues". Measured over 32 archived outlines the floor
fired on **17 of 32** before the fix and **8 of 32** after: nine outlines were told they had no closure
coverage while their own scenes read *"Clearing the Others"* and *"verifying their alibis with staff
logs"*.

That false alarm has teeth. `missing_suspect_closure_scene` drives an outline retry, the repair
guardrail, a deterministic patch that APPENDS clearance language to an Act 3 scene, and a **blocking
abort** at Agent 9. **A wordlist that could not read the outline was manufacturing the duplicate
clearances.**

On 08-19 none of that fired — the outline's own ch10 said "innocent", so the floor was satisfied and
the duplication is Agent 7's primary generation, unprompted:

| ch | title | purpose, verbatim |
|---|---|---|
| 6 | *Clearing the Others* | "Establish alibis and eliminate suspects except Hugo" |
| 10 | *Clearances and Culprit Revealed* | "Clear innocent suspects, confront and reveal Hugo Vane as culprit" |

Agent 7's own outline notes even say *"Suspect clearance scenes concentrated in Act 3, Scene 5"*. It
knew, and allocated three anyway.

**What is built.** `countSuspectClosureScenes` counts the job per scene and reports it as an always-on
`[X32]` warning — free, no behaviour change, and it is the number REVIEW_13 §8.2 had to read a
manuscript to learn. The `duplicate_suspect_closure_scenes` issue and its fold guardrail are gated on
`AGENT9_FOLD_SUSPECT_CLEARANCES`, because **any** coverage issue drives an outline retry and 11 of the
32 archived outlines exceed one scene. The flag now has a trigger where folding is the actual repair.

**What is NOT claimed:** that turning the flag on now fixes pacing. It makes the instruction reachable
for the first time. Whether Agent 7 obeys it is a run.

### 12.9 X63 — a behaviour contract that did not know who it was writing for

Found by the owner, reading an Agent 7 response: every character in a scene carries the *same*
`permittedBehavioursByAct` string. Two different things turned out to be true.

**The outline half is cosmetic.** Agent 7 is told to copy each character's behaviour from its context,
and the context genuinely differentiates them — each line carries that character's own
`Hidden motive:` and `Internal conflict emerging:` clause. Agent 7 copies the generic prefix and
**drops the character-specific half**, in all ten scenes, for every character. It does not reach the
prose, because Agent 9 reads the BUNDLE rather than the outline's copy — but nothing checks it, and
anything that ever starts reading the outline's copy would silently get the flattened version.

**The bundle half is live, and it was role-blind.** `permittedBehavioursByAct` is the only
per-character behavioural steering that reaches Agent 9, printed verbatim as *"Act N behaviour
contract"*. Its comment read *"derived from motive seed + role"*. The code:

```js
const act1 = `...No guilt signals permitted. ${motiveSeed ? `Hidden motive: "${motiveSeed}"...` : ""}`;
const act2 = `May show unease, evasion... One behavioural tell is permitted. ${internalConflict ? ... : ""}`;
const act3 = `Full character reveal permissible. Emotional truth should be explicit — ...`;   // ← reads NOTHING
```

`role` appears nowhere, and could not: `CharacterProfilesResult` has no role field. So on 08-19:

- **Dr. Mallory Finch, the detective**, was instructed that she *"May show unease, evasion, or mild
  defensiveness when questioned. One behavioural tell is permitted."* The detective is not a suspect,
  and in this genre a behavioural tell is exactly the signal that means guilt — the one X49's
  GUILT_MARKER sweep and X59's `shot` idiom work exist to keep off the innocent.
- **Eleanor Voss, the victim**, dead in Act I scene 1, received a live three-act contract whose Act III
  read *"Full character reveal permissible — confrontation, confession, or vindication."*
- **`act3` was byte-identical for all six characters**, and has been on every run ever made.

**What is deliberately NOT fixed.** Culprit and innocent suspects still share one contract. Giving the
culprit its own Act II allowance would hand Agent 9 a behavioural difference the innocents do not
have, which is early disclosure by construction — the defect [§12.x X59](#) spent three reviews
removing. Uniformity among suspects is the feature; uniformity across ROLES was the bug.

**What this is worth.** [REVIEW_13 §2.3](REVIEW_13.md) argues that hook, character clarity and
atmosphere are 8 with no reader complaint and therefore have no lever. This is the first named
mechanism that would produce exactly "correct but unremarkable" characters: the only per-character
behavioural instruction in the pipeline was identical for everyone in Act III and wrong for two roles
in Acts I–II. **It is a mechanism, not a measurement** — nothing here proves it moves the mark, and
only an external read can.

### 12.10 X66 — two facts restated across chapters, and nothing that can compare them

Found by reading `story_20260819-0147` against its own case file ([REVIEW_14 §9](REVIEW_14.md)). Both
instances are in the volatile four — plot, clues, fair play — and neither is a craft defect. Both are
invisible to every check this project has built, for the same underlying reason: **the two statements
of the fact share no token a checker can line up.**

#### (a) The central clue is stated both ways, two chapters apart

| ch | text |
|---|---|
| 1 | *"She glanced at her own watch, then at the wrists of the gathering guests — each timepiece displayed a time that **matched** the clock's face, as if in some silent conspiracy to uphold the illusion."* |
...
Chapter 3 then makes the mismatch the engine of the investigation: *"The contradiction was no longer a
private suspicion but a public anomaly."* Chapter 1 had already ruled it out.

**Why nothing sees it.** The X38 / X39 / X44 family compares CLOCK VALUES, and neither sentence
contains one — *"matched the clock's face"* and *"none displayed the same time"* are assertions about
a RELATIONSHIP, not about a time. `parseClockTime` returns null on both, and every gate built on it
degrades to silence. Nothing in the pipeline tracks whether a relationship asserted in one chapter is
asserted with the opposite polarity in another.

This is the most consequential single defect in the manuscript. The whole deception rests on the
guests' watches disagreeing with the clock; chapter 1 tells the reader they agree.

#### (b) One locked scalar, spent as three different dimensions

`wire_length = "twenty-five feet"`, described in the registry as *"Length of the thin control wire
from rooftop terrace to clock mechanism"*.

```
ch1, ch4   the wire's LENGTH        "…came to twenty-five feet."
ch3        a HORIZONTAL distance    "…the Rooftop Terrace door some twenty-five feet away"
ch9        a VERTICAL height        "…the thin control wire from the Rooftop Terrace, twenty-five feet above"
```

Three different physical quantities, all equal to the one number the registry pinned. It is not
impossible, only implausible — and it is plainly an artifact of a locked scalar being reachable for
any measurement the prose needs.

**Why nothing sees it.** The locked-fact consistency gate is deliberately value-agnostic: it checks
that the STRING appears, and all three uses satisfy it. But **the registry already knows the
dimension** — the description says *length*, *of the wire*, *from A to B*. [X65](#) built
`measurementSubject` to parse exactly that description for exactly that structure. The information is
present and unused.

#### Not built, and deliberately so

[REVIEW_14 §7](REVIEW_14.md) lists *more detectors on plot, clues and endings* as **not the path**:
§2.1 is a reliability problem, not a detection one, and this project has repeatedly found that a new
detector on the volatile four buys less than fixing the instruments that already exist. Writing a
polarity tracker for (a) would be the fourth generation of exactly that mistake.

This row exists so the defect has an owner, a measurement and a name — not so that something gets
built next. If it is ever built, **(b) is the cheaper and better-posed half**: a dimension check
against a description the codebase already parses, with a bounded false-positive surface. **(a) has
no cheap shape** — "did a relationship flip polarity between chapters" is a semantic question, and
the honest answer today is that only a reader catches it.

**What would change this ordering:** an external read naming the watch contradiction. It is the kind
