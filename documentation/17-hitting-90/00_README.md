...
| P3.1 | the fatal wound placed where it kills | measured: 3 of 3 harness cases name a site — all three "to the heart", the list's second item (A_67: the list is copied) | 357129c2 | seed 23403's case said "stabbed with a fencing foil" and a clue said "torn sleeve"; both engines wrote an arm wound from the sleeve |
| P3.2 | an opportunity stated as a window with two ends | measured: 3 of 3 harness cases carry the act window; the note fired once on the PROBE ("could be killed" vs "killing"), widened and pinned | 357129c2, (this commit) | MEASURED: 18 of 67 archived cases carry no window about the act; seed 23403 had one, so its reads' complaint is P1.4's |
...
| P4.1 | v2's editor pass targets the register lines readers quote | built, measured on pair 3: abstract-subject sentences 6 → 1 | caaedcf0 | `abstract_subject` finding; known-positive test |
| P4.1b | the wit operation narrated as it is performed ("spoke at length" ×7, "answer came in four words", "first exchange") | built after pair 3; not yet in a book | (this commit) | `operation_narrated` finding; pair 3's four lines are the known positives |
| P4.2 | our own labels ("polite savagery") never reach the writer as quotable text | measured on pair 3: 16 → 1, and the one is ordinary English about the victim | caaedcf0 | MEASURED on pair 2 (before the fix): 16 humour labels printed in the book — polite savagery 5, deadpan 6, dry wit 3, observational humour 2. `humourMove` describes the move; the label is in no prompt |
