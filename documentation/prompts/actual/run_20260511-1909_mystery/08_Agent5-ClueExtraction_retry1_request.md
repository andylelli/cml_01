...
- Project ID: `unknown`
- Timestamp: `2026-05-11T19:10:33.855Z`
...
1. Generate a clue that makes the reader directly observe: "The clock in the study shows ten minutes past eleven when examined...."
   → observation | essential | early | temporal | step 1 | terms: clock, study, shows

2. Generate a clue that provides evidence for: "This indicates a false time of death as the clock was tampered with...."
   → contradiction | essential | early | temporal | step 1 | terms: indicates, false, death

3. Generate a clue that makes the reader directly observe: "An unusual scratch on the clock's winding mechanism suggests tampering...."
   → observation | essential | early | temporal | step 2 | terms: unusual, scratch, clock's

4. Generate a clue that provides evidence for: "The scratch indicates that the clock was manipulated recently...."
   → contradiction | essential | early | temporal | step 2 | terms: scratch, indicates, clock

5. Generate a clue that makes the reader directly observe: "Witnesses recall hearing the clock chime at a different time than indicated...."
   → observation | essential | mid | temporal | step 3 | terms: witnesses, recall, hearing

6. Generate a clue that provides evidence for: "This discrepancy reveals the clock's tampering and undermines the reliability of witness accounts...."
   → contradiction | essential | mid | temporal | step 3 | terms: discrepancy, reveals, clock's

7. Generate one essential early or mid contradiction clue that explicitly overturns the false assumption with reader-visible evidence before the discriminating test. Anchor: "This indicates a false time of death as the clock was tampered with...."
   → contradiction | essential | mid | temporal | step 1 | terms: indicates, false, death

8. Generate at least one essential early or mid observation clue that makes the core mechanism reader-visible before the discriminating test. The clue must surface this mechanism detail concretely: "A mechanical clock was tampered with to mislead witnesses about the time of death...."
   → observation | essential | early | temporal | terms: mechanical, clock, tampered

9. Generate a clue that provides observable evidence the reader must see BEFORE the discriminating test can be understood. The test exploits this evidence — it does NOT reveal it. Evidence for: "A controlled demonstration is set up where the clock's true timing is compared against witness state..."
   → observation | essential | mid | temporal | terms: controlled, demonstration, clock's
...
10. Generate one essential mid-story clue whose description or pointsTo explicitly names Dr. Mallory Finch and states the unique trace, preparation detail, or mechanism link that points to Dr. Mallory Finch rather than any non-culprit.
   → observation | essential | mid | temporal | terms: Dr. Mallory Finch, mechanical, clock, tampered
...
15. Generate one essential early or mid elimination clue that explicitly rules out Eleanor Voss and narrows the solution toward culprit Dr. Mallory Finch. The pointsTo text must start with "Eliminates Eleanor Voss because ..." and cite corroborated evidence.
...
- Clock time showing ten minutes past eleven
- Witness statements about time of last sighting
- The clock was tampered with to show the wrong time
...
## Physical Evidence
- Scratches on the clock's winding mechanism
...
Create 2 red herrings that support the false assumption: "The time of death can be accurately determined through witness accounts."
...
Violations: 1. [critical] Source Path Legality: Invalid source path: (empty-source-path)
Warnings: Late clue count 0 is below preferred minimum 1
Recommendations: Move essential clues to early/mid placement; Avoid private/detective-only clue phrasing; Ensure clue IDs are unique and timeline is balanced; Repair path exactly: (empty-source-path) -> use one of [CASE.inference_path.steps[N].observation | CASE.inference_path.steps[N].correction | CASE.inference_path.steps[N].required_evidence[M] | CASE.constraint_space.time.anchors[M] | CASE.constraint_space.time.contradictions[M] | CASE.cast[N].alibi_window | CASE.cast[N].access_plausibility | CASE.constraint_space.physical.traces[M]]; Strict first-attempt contracts are active: apply strict source whitelist and required ID->source mappings exactly.; Do not defer strict mapping and direct culprit evidence requirements to retry mode.; RETRY MODE — You MUST return valid clues with correct sourceInCML paths. Returning status=fail or clues=[] is not acceptable on this attempt. Choose any valid path from the template list above.
...
- If any target mentions red-herring overlap, include rewrite table entries as: old phrase -> replacement phrase.
    ...
    - violation_codes[]:
      - none
- must_fix[]:
  - [critical] Source Path Legality: Invalid source path: (empty-source-path)
  - [warning] Late clue count 0 is below preferred minimum 1
    - targeted_clue_ids[]:
      - none
    - preserve_clue_ids[]:
      - none
...
- required_direct_culprit_clue:
  ...
  - allowed_source_paths=CASE.cast[1].access_plausibility, CASE.cast[1].alibi_window, CASE.cast[1].evidence_sensitivity[0], CASE.inference_path.steps[1].observation, CASE.inference_path.steps[1].correction, CASE.inference_path.steps[2].observation, CASE.inference_path.steps[2].correction
