...
- Project ID: `unknown`
- Timestamp: `2026-05-16T21:01:00.348Z`
...
Violations: 1. [critical] Discriminating Test ID Contract: Missing clue id(s): clue_dust_marks, clue_chime_report, clue_pendulum_disturbance
Warnings: None
Recommendations: Every CASE.discriminating_test.evidence_clues ID must exist exactly in clues[].id: clue_dust_marks, clue_chime_report, clue_pendulum_disturbance; Set these required IDs to criticality=essential and placement=early|mid.; Preserve unaffected clue IDs and wording unless dependency requires change.; Keep sourceInCML paths legal and in-range.; Strict first-attempt contracts are active: apply strict source whitelist and required ID->source mappings exactly.; If any strict contract cannot be satisfied, return status=fail with unresolved blockers in audit.invalidSourcePaths.; Do not defer strict mapping and direct culprit evidence requirements to retry mode.
...
- If any target mentions red-herring overlap, include rewrite table entries as: old phrase -> replacement phrase.
    ...
    - violation_codes[]:
      - none
- must_fix[]:
  - [critical] Discriminating Test ID Contract: Missing clue id(s): clue_dust_marks, clue_chime_report, clue_pendulum_disturbance
    - targeted_clue_ids[]:
      - none
    - preserve_clue_ids[]:
      - none
