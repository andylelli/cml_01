...
- Project ID: `unknown`
- Timestamp: `2026-05-20T18:10:41.140Z`
...
2. Generate a clue that provides evidence for: "The clock's time does not match the expected time of death...."
   → contradiction | essential | early | temporal | step 1 | terms: clock's, match, expected

3. Generate a clue that makes the reader directly observe: "Witness statements indicated dinner was served at 8:00 PM...."
   → observation | essential | early | testimonial | step 2 | terms: witness, statements, indicated

4. Generate a clue that provides evidence for: "This creates a contradiction with the clock's time...."
   → contradiction | essential | early | temporal | step 2 | terms: creates, contradiction, clock's

5. Generate a clue that makes the reader directly observe: "The clock's casing shows signs of tampering...."
   → observation | essential | mid | temporal | step 3 | terms: clock's, casing, shows

6. Generate a clue that provides evidence for: "Dr. Finch had the opportunity to tamper with the clock...."
   → contradiction | essential | mid | temporal | step 3 | terms: finch, opportunity, tamper

7. Generate one essential early or mid contradiction clue that explicitly overturns the false assumption with reader-visible evidence before the discriminating test. Anchor: "The clock's time does not match the expected time of death...."
   → contradiction | essential | mid | temporal | step 1 | terms: clock's, match, expected

8. Generate at least one essential early or mid observation clue that makes the core mechanism reader-visible before the discriminating test. The clue must surface this mechanism detail concretely: "A mechanical clock was wound back to mislead the investigation...."
   → observation | essential | early | temporal | terms: mechanical, clock, wound

9. Generate a clue that provides observable evidence the reader must see BEFORE the discriminating test can be understood. The test exploits this evidence — it does NOT reveal it. Evidence for: "A controlled comparison of the clock's time against another accurate clock proves the tampering...."
...
10. Generate one essential mid-story clue whose description or pointsTo explicitly names Dr. Mallory Finch and states the unique trace, preparation detail, or mechanism link that points to Dr. Mallory Finch rather than any non-culprit.
   → observation | essential | mid | temporal | terms: Dr. Mallory Finch, mechanical, clock, wound

11. Generate a clue showing observable evidence of Dr. Mallory Finch's premeditation or planning (Unrequited love and financial gain). This MUST be visible to the reader before the confrontation scene — the detective cannot privately know this and withhold it.
   → observation | essential | mid | behavioral | terms: unrequited, financial
...
- Captain Ivor Hale: sensitive items: Travel documents
...
Create 2 red herrings that support the false assumption: "The time of death must align with the dinner timeline."
...
Violations: 1. [critical] Inference Path Coverage: Discriminating test evidence_clues reference missing clue id(s): clue_clock_time_stopped, clue_witness_dinner_time, clue_clock_tampering_signs; 2. [critical] Inference Path Coverage: Discriminating test references no evidence found in the clue set
Warnings: Inference step 1 has no contradiction clue; Inference step 1 has clues but no contradiction/elimination evidence
...
- If any target mentions red-herring overlap, include rewrite table entries as: old phrase -> replacement phrase.
    ...
    - violation_codes[]:
      - none
- must_fix[]:
  - [critical] Inference Path Coverage: Discriminating test evidence_clues reference missing clue id(s): clue_clock_time_stopped, clue_witness_dinner_time, clue_clock_tampering_signs
  - [critical] Inference Path Coverage: Discriminating test references no evidence found in the clue set
  - [warning] Inference step 1 has no contradiction clue
  - [warning] Inference step 1 has clues but no contradiction/elimination evidence
    - targeted_clue_ids[]:
      - none
    - preserve_clue_ids[]:
      - none
...
- required_id_to_source_mappings[]:
  - clue_clock_time_stopped -> CASE.discriminating_test.evidence_clues[0]
  - clue_witness_dinner_time -> CASE.discriminating_test.evidence_clues[1]
  - clue_clock_tampering_signs -> CASE.discriminating_test.evidence_clues[2]
  - clue_culprit_direct_dr_mallory_finch -> CASE.cast[1].access_plausibility
  - clue_victim_time_of_death -> CASE.prose_requirements.clue_to_scene_mapping[3].clue_id
  - clue_travel_documents -> CASE.prose_requirements.clue_to_scene_mapping[4].clue_id
...
- required_direct_culprit_clue:
  ...
  - allowed_source_paths=CASE.cast[1].access_plausibility, CASE.cast[1].alibi_window, CASE.cast[1].evidence_sensitivity[0], CASE.inference_path.steps[2].observation, CASE.inference_path.steps[2].correction
