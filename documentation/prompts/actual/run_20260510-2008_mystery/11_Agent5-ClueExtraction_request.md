...
- Project ID: `unknown`
- Timestamp: `2026-05-10T20:10:31.235Z`
...
8. Generate at least one essential early or mid observation clue that makes the core mechanism reader-visible before the discriminating test. The clue must surface this mechanism detail concretely: "The mechanism relies on clock, alibi, and faint to expose the false timing...."
   → observation | essential | early | temporal | terms: mechanism, relies, clock,

9. Generate a clue that provides observable evidence the reader must see BEFORE the discriminating test can be understood. The test exploits this evidence — it does NOT reveal it. Evidence for: "A controlled demonstration re-enacts the timeline using the clock to prove its tampering...."
   → observation | essential | mid | temporal | terms: controlled, demonstration, re-enacts
...
Violations: 1. [critical] Information Parity: The detective knows details about the clock's mechanism that the reader does not.; 2. [critical] Logical Deducibility: The reader cannot follow the deduction chain due to unclear connections between clues.; 3. [critical] Discriminating Test Timing: The discriminating test relies on evidence not sufficiently established earlier.; 4. [critical] Solution Uniqueness: Insufficient elimination of other suspects before the reveal.
Warnings: Consider enhancing character motivations to strengthen the narrative's engagement.; First-pass acceptance contract: essential early|mid mechanism, contradiction, and elimination clues; reader-solvable observation -> correction -> elimination chain before Act III.; Unified retry contract: keep cast-path coupling, discriminating evidence ID parity, and mechanism-visibility ordering non-conflicting in one output.
...
- REQUIRED CLUE CONTENT (must be covered by essential early/mid clues):
  ...
  - solution_uniqueness
- must_fix[]:
  - [critical] Information Parity: The detective knows details about the clock's mechanism that the reader does not.
  - [critical] Logical Deducibility: The reader cannot follow the deduction chain due to unclear connections between clues.
  - [critical] Discriminating Test Timing: The discriminating test relies on evidence not sufficiently established earlier.
  - [critical] Solution Uniqueness: Insufficient elimination of other suspects before the reveal.
  - [warning] Consider enhancing character motivations to strengthen the narrative's engagement.
...
- required_id_to_source_mappings[]:
  ...
  - clue_mechanism_visibility_core -> CASE.prose_requirements.clue_to_scene_mapping[2].clue_id
  - clue_4 -> CASE.prose_requirements.clue_to_scene_mapping[4].clue_id
  - clue_5 -> CASE.prose_requirements.clue_to_scene_mapping[5].clue_id
  - clue_core_contradiction_chain -> CASE.prose_requirements.clue_to_scene_mapping[6].clue_id
