# Agent 65 - World Builder Prompt (Runtime-Accurate)

Use with: `documentation/prompts/PROMPT_BACKBONE_STANDARD.md`

Source of truth:
- `packages/prompts-llm/src/agent65-world-builder.ts`
- `schema/world_document.schema.yaml`

This document captures what is actually sent to the LLM by runtime code.

## Message Sequence Sent To LLM
1. `system`: `WORLD_BUILDER_SYSTEM`
2. Optional retry preamble:
...
4. On attempts 2/3 after failure: extra `user` correction message with explicit failed validation error and mandatory checks.
...
## User Prompt Builder (Exact Structure)

Notes:
- `ARC_DESC_PROMPT` is dynamic at runtime from generation params:
	- `ARC_DESC_PROMPT = arc_description_gate + arc_description_prompt_buffer`
- Inputs are serialized with `JSON.stringify(..., null, 2)`.
...
When validation fails, runtime appends this correction-style user prompt (with injected error text):
...
## Enforcement Notes
- `jsonMode: true` is enabled on chat call.
- Output is still post-validated by schema (`world_document`) and additional runtime gates.
- Runtime validation is the final authority if this doc drifts.
