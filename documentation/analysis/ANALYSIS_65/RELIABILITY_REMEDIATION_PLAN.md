...
| **5** | Post-Phase-1-probe re-rank (R1): measure what the inversion killed; fix top residual | ~2–4h | A_65 Phase-1 probe + Phase 3 | ✅ **done — the re-rank, by the decision rule** (cut ≥50% → leave alone; flat → independent root): regens **5–16→1, cut >90%** by the inversion+plants → leave alone · synthesis lines **~12→3, cut ~75%** → leave, watch convergence · Agent-9 retries 6.2→4 (−35%, borderline — watch) · **AtmosphereRepair FLAT 7.6→8 → independent root confirmed: genuine model repetition** (census said the loop never converges) → routed to craft Phase 3 drafting-variety, NO more repair/detector work (anti-mole law) · corr% 47→35 (target <30 in sight). One probe, both plans read | 2026-07-23 |
| **6** | Source-layer clue completeness (R2b): kill the every-run synthesis; herring separation at source | ~3–5h | Phase 2 ranking | ✅ done — **root cause found precisely: the strict contract was RETRY-ONLY and retries are off by default** (verified: 0 strict lines in the probe's single Agent-5 request). New `strictContract` input renders the exact shapes FIRST-PASS (required ids→sources, direct-culprit clue, late slot, step floors) with scene-register instruction ("never meta-boilerplate"); worker passes `buildStrictPromptFeedback(cml)` unconditionally; synthesis machinery stays as the counted floor. Herring separation already first-pass (FIRST-ATTEMPT RED HERRING CONTRACT + forbidden terms) — held to the warn-band count. Fixtures 2 (renders with, silent without) | 2026-07-23 |
| **7** | Detector census over the 93-story corpus + precision-fixture retrofit (R3) | ~2–3h | Phase 3 | ✅ done — `scratchpad/a65/detector-census.{mjs,tsv}`, ERA-SPLIT (91 stories): **scaffold 47%→46%→0%, template-leakage 59%→23%→0%, report-style 29%→10%→0% — the fixes hold and those detectors are precise.** Two current-era flags: **recurring-phrases fires on 100% of shipped stories in EVERY era including post-repair — the AtmosphereRepair loop does not converge** (hardens the Ph5 candidate: demote to measure, fix variety at drafting); evidentiary-register RISING 21%→33%→40% (the live craft residual — A_65 F3's target, a measure by design) | 2026-07-23 |
| **8** | Input-integrity audits (R4): JSON boundaries + `repairCaseSoundness` vs Type C | ~2–3h | — | ✅ done — inventory found **five boundaries running jsonrepair with NO truncation guard, including Agent 3 (the CML itself)**. Shared kit: `shared/json-boundary.ts` (`looksTruncatedJson` + `guardedJsonrepairParse`; the rule: sloppy-but-closed may repair, TRUNCATED is REFUSED — never ingest a phantom). Applied to agent3-cml, agent6-fairplay, agent65-world-builder, agent2d, agent2e (agent5 already had it); 7 kit fixtures incl. the a3c2973f shape. **Type-C checklist: every class has an arm + fixtures** (dual-detective, dual-victim #11, culprit-is-victim, culprit-eliminated, suspect-unaccounted in `repairCaseSoundness`; victim-coverage and beat-enum tested at their own layers) — no gaps | 2026-07-23 |
