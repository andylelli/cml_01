...
| **1** | The Withheld-Inference build: inventory → invert → license → guard → fixture | ~4–6h | reliability Ph1 (preflight) helpful, not required | ✅ **done** — inventory found the mandate in BOTH obligation-block branches + two prompt-builder "Points to:" sites (G6/deducibility/pre-commit gates verified observation-keyed; regen instructions already observation-first; clearances keep their bounded micro-license; only ABSENCE test-pins existed). Inversion: ¶2 mandate deleted both branches → OBSERVATION ONLY (notice, never explain); Points-to suppressed pre-reveal, kept at the licensed beats; prompt-builder sites relabeled "Later significance (context for YOU — do NOT state…)". Licensing: false solution GAINS "REASONING LICENSED — honestly wrong"; DT + walked reveal keep theirs; every other pre-reveal chapter carries the INFERENCE EMBARGO. Measures: `detectEvidentiaryRegister` (frozen family, narration-scope) + `detectDerivedContradictionLeak` (the F3 ch1 sentence, fixture-narrow, both canonical values + assembly) in prose-guard; both wired as ship-check warn lines (never gates); measures TSV single-sources the register detector. **NET-SMALLER LAW HOLDS: baseline 39 lines → asserted < 39 in CI.** Fixtures: 6 prose-guard + 4 contract + net-smaller. prompts-llm 813 · prose-guard 49 · worker 348 · tsc 0 · preflight green | 2026-07-23 |
| **2** | Probe #1 (the shared spend) + the measures read | ~£1.5 | 1 (+ reliability Ph3 for the TSV) | ✅ **done** — `mystery-1784751055781` (clock), shipped+scored, **zero caps**. THE INVERSION HOLDS ON THE PAGE: inference density flattened to 1/0/0/0/0/1/1/1/0/1 (was serialized spikes), NO derived-contradiction leak fired, regens **collapsed 5–16→1** (scaffold-regen demand gone), corrective share 47%→**35%**, warn band 19–40→**8**, register hits 1. Synthesis 12→3 lines (first-pass contract working, not yet converged). AtmosphereRepair FLAT at 8 — now chasing GENUINE model repetition ("silence was broken only by the distant…"), routed to Phase 3 variety, not repair work. Rubric **69** recorded (corpus #3 ever, n=1, not concluded) | 2026-07-23 |
| **3** | Top-6 profile: derive the live-category deltas → encode 2–3 contracts | ~3–4h | derivation free now; contracts ride probe #2 | ⬜ | — |
| **4** | Instrument decision: offset branch A/B (+ optional external re-read of probe #1) | owner | 0 | 🟡 **the probe-#1 external read is IN** (`stories/story_20260722-2127/chatgpt-review.txt`): **78/100 external vs 69 internal — offset +9, replicating S0's +9.5.** Category contrast PROVES the dead-band thesis on one story: internal clues 5 / pacing 6 vs external **clues 8 / pacing 8 / plot_structure 8 / ending 8** — the reviewer explicitly praises the fair-play contradiction, the false solution, and the earned reveal (the A_64+Ph1 levers moved the REAL experience; the internal judge could not see it). **Branch A stands: internal ~73 is the proxy; dead-bands ignored.** Reviewer's ceiling: "with pronoun drift fixed… 84–87" — the first external sighting of 80+ territory. Remaining Phase-0 reads (4 M3 stories) still valuable for offset confidence | 2026-07-23 |
| **5** | Standing measures (joint harness — reliability Ph3 owns the build; craft metrics ride in it) | (shared) | reliability Ph3 | ⬜ | — |

**THE PROBE-#1 EXTERNAL REVIEW WORK QUEUE (2026-07-23, evidence-ranked):**
1. **PRONOUN ROLE-CONTAMINATION — the top craft blocker** (external: prose 5, character_clarity 6, "severe"): the model assigns pronouns by the ROLE's historical occupant, not the character holding it (investigator drifted to she/her because prior drafts used a female investigator). The existing lock/sweep/repair stack misses it at scale. The reviewer: fixing this alone reaches 84–87. Next sitting's work item — root-cause the pronoun stack against this story's drift instances (all in the artifact + prompt archive).
2. ✅ A3 honorific blind spot — FIXED same-day (periods allowed in the span; "Dr. Mallory Finch" fixture verbatim; patch supportClause sanitized).
3. Plant the TRUE-TIME VALUE early (the watch's 8:30 stated in ch2, paid off at the reveal) — a C1 refinement: value-bearing observables should carry their value at the plant.
4. False-solution physical clue — the false suspect needs one disprovable PHYSICAL clue, not just emotional plausibility (Agent-5 requirement candidate).
5. Method tell at discovery — strangulation showed no early ligature clue despite RC3.5 ON; check the tell-token map for strangulation coverage.
6. Body-discovery-delay plausibility — if death precedes the false time, the story must account for the gap (CML soundness candidate).
