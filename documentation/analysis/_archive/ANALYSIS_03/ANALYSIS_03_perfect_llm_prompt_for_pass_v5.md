# ANALYSIS_03 Perfect LLM Prompt (Pass-First, v5)
...
### B.3) Source Path Format Contract (Critical)
1. Use bracket-index notation only for indexed segments.
2. sourceInCML must point to a terminal leaf field, not an intermediate node.
3. Do not emit dot-index forms.
...
Invalid examples:
- CASE.inference_path.steps.0
- CASE.inference_path.steps.1
- CASE.inference_path.steps[1]
- CASE.cast.3.alibi_window
...
- Emit full clue objects only; do not emit partial clue objects.

### D.3) Discriminating ID Exactness Contract
- Every id from DISCRIMINATING_EVIDENCE_IDS must appear exactly, preserving punctuation and underscores.
- Do not normalize, rewrite, or simplify IDs (for example clue_1 -> clue1 is forbidden).
...
- status MUST be "fail" if any sourceInCML uses malformed path format (dot-index, missing leaf, or non-bracket index).
...
### E.2) Anti-Collapse Output Rule (Critical)
- status="fail" does NOT permit empty clues[] when CML evidence exists.
- Even when failing hard contracts, output a best-effort full clue set with required fields populated.
- Best-effort full clue set still requires complete clue schema fields (id, category, description, sourceInCML, pointsTo, placement, criticality, supportsInferenceStep, evidenceType).
- Keep unmet contracts in audit arrays; do not satisfy fail-fast by returning empty arrays.
- clues[] may be empty only if CML input is unusable or contains no clue-derivable evidence.
...
- Use sourceInCML bracket-index leaf paths only; never use dot-index or intermediate-node paths.
- Preserve discriminating evidence IDs exactly (including underscores) when emitting clue IDs.
...
- Do not return empty clues[] when evidence exists; return best-effort clues and set status="fail" with populated audit arrays.
...
- discriminating evidence IDs are exact string matches (no normalization)
- essential solving clues only in early or mid
- cast path binding valid
- sourceInCML validity exact-match
- sourceInCML uses bracket-index leaf format (no dot-index or intermediate nodes)
...
- every clue object includes all schema fields (no partial clue objects)
- every inference step has mapped clue coverage
- every inference step has contradiction coverage
- every named non-culprit suspect has elimination/alibi evidence parity
- top-level keys are exactly status, clues, redHerrings, audit
- if any check fails, set status="fail" and report blockers
- if checks fail, still output best-effort non-empty clues[] unless CML evidence is unusable
