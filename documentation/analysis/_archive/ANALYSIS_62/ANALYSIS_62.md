...
## TRACKER — state as of 2026-07-19 (see the ledger for the numeric authority)

**The remaining path now lives in [ANALYSIS_63](../ANALYSIS_63/ANALYSIS_63.md)** — it supersedes this doc's "THE ROAD TO 80" section (kept below for history).

**P3 CLOSED** (scaffold ON `6d44f800` · leakage NO-FLIP, cap source-fixed `3cfdc7af` · dual-value inconclusive-deferred) → **P4 gate SAT 2026-07-19** (no new default flips; injector deletion deferred by its own evidence rule; buy-back chose reveal — but see the M2 row: reveal fired 0/4 on the check batch, re-read before funding) → **M2 CHECK BATCH RUN 2026-07-19: 4/4 shipped, £0.98, leakage cap 19/39→0/4 (source fix validated live), caps criterion met except `dualValueNoContrast` 2/4; median 64 vs ≥73 → the residual is RAW CRAFT** → **P5 (craft levers) is the live phase**, with the dual-value fresh-run A/B as the open cap question. The two Jul-17-evening runs died to host death (stamped INTERRUPTED, RC-5 machinery). `AGENT9_CRITIQUE_REWRITE=true` confirmed live in `.env.local` — the P5.1 control arm must force it off.

## TRACKER (historical) — state as of 2026-07-17 05:40
...
| 9 | **M2-era first (2026-07-19, P5-CR clock arm):** locked-fact/clue time comparator did an ABSOLUTE-minutes diff, so a meridiem-less word-form fact ("ten minutes to nine" = 8:50) vs an explicit-pm clue time ("8:00 pm") — 50 real minutes apart — scored 670 min → CML-integrity hard abort pre-prose. The FP pair: Agent-6's pre-LLM backstop had copied the inference-step text (with its time) into a fabricated contradiction clue, and the abort path used the OLD comparator while the warning path already had the 12-hour-circle fix (`classifyTemporalDifference`) — the split-brain pattern of class #5 again, this time between two severities of the same check | `detectLockedFactClueTimeMismatch` now delegates to `classifyTemporalDifference` (12-hour circle on asymmetric/absent meridiem); 6 both-directions fixtures incl. the exact aborting pair; power case (11:30 pm vs 8:50) still aborts | ✅ fixed same-day; validated on the aborting theme+arm (P5-CR clock_on rerun `mystery-…9072380` shipped, gate `warning`, 2026-07-19) |
| 10 | **The victim-role predicate is a substring match, ~30 sites, and it poisoned GENERATION:** CML gave Beatrice Quill `role_archetype: "Friend of the victim"` → every `role.includes("victim")` site agreed she was the deceased — the prompt builder told the model "Beatrice Quill — DECEASED, past-tense only", the lifecycle validator convicted her living scenes, the rescue pass reframed 5 of them → `victim_reappears_alive` + `deceased_character_confesses` reader-trust criticals → gate `failed` (P5-CR poison arm, `mystery-…0757946`, rubric 62 wasted). Word-boundary matching cannot fix this ("friend of the *victim*" contains the whole word); `identity-match.ts` had documented the FP class without covering it | `isVictimArchetype` in `@cml/cml` (head-noun semantics: victim must be what the role DENOTES — "murder victim" ✓, "friend of the victim" ✗, "victim's confidant" ✗, "victim advocate" ✗); ALL ~30 sites swept across worker/prompts-llm/story-validation (RC-6.2 choke-point lesson: one predicate, one home); 14 fixtures both directions; prompts-llm 768 + story-validation 414 + worker 325 green | ✅ fixed same-day; validated on the aborting theme+arm (P5-CR poison pair re-paired on the fixed dist: off `mystery-…2209178` + on `mystery-…3065999`, both shipped gate `warning`, 2026-07-19) |
| 11 | **Class #3's sibling, victim-side (P5-DV poison arm, `mystery-…4957442`):** Agent 3's CML gave TWO cast members `role_archetype: "Victim"` (Beatrice Quill + the designed victim Sylvia Trent) — the head-noun predicate correctly read both as denoting, both were prompted "DECEASED, past-tense only", the model wrote the TRUE victim walking into the pantry, and `victim_reappears_alive` + `cleared_culprit_conflict` failed the gate. `repairCaseSoundness` demoted duplicate DETECTIVES (class #3, `6dff6933`) but had no victim arm | duplicate-victim repair in `repairCaseSoundness` (§8b), same design-authority precedence: keep Agent 2's designed victim (role/roleArchetype) or `culpability.victim`, demote the rest to their designed role; 4 fixtures incl. the exact aborting cast + relational-role non-double-count; suite 19 green | ✅ fixed same-day; validated on the aborting theme+arm (P5-DV poison_on rerun `mystery-…2735935` shipped, gate `warning`, 2026-07-20 — this run's cast drew a single victim so the repair path stayed idle; the fixture suite carries the exact aborting cast) |
| 12 | **Class #8's inverse — hygiene destroys the late plant (P5-DV poison, `mystery-…0276364`):** the class-#5 NSD-anchor repair planted `clue_6`, the gate's own matcher ACCEPTED it chapter-locally, then `applyStandardPostProcessingChain` rewrote the planted paragraph (name-hygiene family) and the post-hygiene re-collect missed → hard-stop with no second attempt. The code's own NOTE had flagged this residual. Trigger context: a degenerate template-ish clue description ("poison, victim, and floor") synthesized from the inference path | caller now runs TWO rounds (plant → full hygiene → re-collect each time); round 2's regen instruction forbids naming/titling any person (the anonymiser/walk-on rewriters are the destructive passes) and hygiene-destruction is logged by name ("accepted by the matcher but DESTROYED by the post-processing chain"); `detailSuffix` plumbing pinned by 2 fixtures (6 green); no-plant short-circuit avoids wasted round-2 spend | ✅ fixed same-day; validated 2026-07-20: the aborting theme+arm (tide_on `mystery-…8387267`) shipped gate `warning` on the fixed dist — this run drew no unanchored clue so the round machinery stayed idle; the mechanics are fixture-pinned (9 green) |
| 13 | **The content filter starves a repair pass (P5-DV tide_on, `mystery-…6986525`):** the class-#12 rounds engaged as designed, but BOTH regen attempts died on the Azure content filter ("response was filtered…", a drowning-theme chapter) — zero candidates, and the no-plant short-circuit correctly declined round 2 → certain hard-stop. An LLM-only repair has no floor when the provider refuses to answer | deterministic floor in `repairUnanchoredNsdCluesBeforeGate`: append a neutral-subject paragraph carrying the clue observable verbatim ("The record now held…" — the production builder's detector-tested phrasing; class-#6 name/term separation honored), accept iff THE GATE'S OWN matcher sees it; a flat sentence risks a rubric cap and a cap beats an abort. 3 fixtures (starved→floor-rescue, matcher-rejects→honest hard-stop, empty-observable→no empty plant); one superseded class-#5 fixture updated to the new contract; suite 9 green | ✅ fixed same-day; validated 2026-07-20: tide_on rerun (`mystery-…8387267`) shipped gate `warning` — no starved-regen event occurred on this run (floor idle); starved→floor-rescue, matcher-reject, and empty-observable paths are fixture-pinned |
| 14 | **One retry was the last word at the outline pacing gate (P5-VOICE poison_enforce):** a catastrophic Agent-7 outline draw (2/10 clue-bearing scenes vs ≥6) survived one guardrailed retry at 2/10, and the deterministic fill (craft-capped at 3 synthetic scenes) could not bridge a gap of 4 → pre-prose abort. The run died for want of ONE more ~$0.05 Agent-7 call | second targeted retry when the gap exceeds the fill cap (harder guardrail names the exact counts; same acceptance ladder: pass → accept, small-gap → bounded fill, still short → the abort stands); agent7 suites 18 green, tsc clean | ✅ fixed same-day; validated on the aborting theme+arm (P5-VOICE poison_enforce rerun `mystery-…2686670` shipped gate `warning`, 2026-07-20) |
...
| **P3 — Measure it** | 🟡 **IN FLIGHT (second firing)** | **First firing: 16/16 arms dead, $0 spent, analyzer refused all verdicts** — the replay harness hadn't run since the pipeline evolved, and hydrated artifacts bypass live-path normalization (RC-2's disease in a new organ). Three fixes (`3739a1c7`): object-shaped cast candidates in the G2 check; missing `redHerrings` normalized at BOTH fair-play entry points; boundary now prints stacks. **Known residual:** no archive carries `coverageResult`, so every replay synthesizes a fresh Agent-6 audit — Jul-14-era archives can fail its structural read (45/100 hard-stop; killed 3 scaffold pairs), a **replay-only distortion**. Mitigation staged: top-up pool of 8 hydratability-verified M1v9/M1v10 scaffold-cap runs; N≥4 floor enforced before any verdict is read. |
