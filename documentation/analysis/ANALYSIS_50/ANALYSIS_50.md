...
### §9.4 — Structural fix #1 LANDED (2026-06-26, commit `89402c64`)

**Fix #1 (constrain Agent 3b to the locked theme) is implemented + unit-tested.** Flag-free correctness (no-op for open themes), three parts:
- **Prompt:** when the theme commits to a concrete mechanism family (`extractThemeMechanismFamilies` — keyword map over the theme text; the real signal, since `deriveHardLogicDirectives` only yields abstract axis families like "schedule contradiction"), the developer prompt now requires the **PRIMARY device (devices[0]) to realize that family**, and the "explore many families / do not default to the clock" de-anchoring is scoped to **secondary** devices only.
- **Deterministic:** `selectThemeCoherentPrimary` reorders so the theme-matching device leads — applied in `generateHardLogicDevices` to the initial set **and every plausibility-regen candidate**, so the locked-fact registry (built from `devices[0]`) is theme-coherent. No device is dropped/mutated.
- **Regen guard:** the `AGENT3B_PLAUSIBILITY_JUDGE=enforce` path **never trades a theme-coherent primary for a higher-plausibility off-theme one** (and prefers recovering theme-coherence); a telemetry warning fires when a locked theme produced no matching device. Never aborts.
- **Tests:** `agent3b-theme-lock.test.ts` (14) green; prompts-llm 499 / worker 212 green; type-checks clean. Offline-verified on the real canary theme → families `["clock"]`, prompt emits the LOCKED-THEME section.
- **Acceptance (pending live run):** on the next levers-on canary, 3b's primary device + `lockedFacts` must be clock-tampering (telemetry `matchedThemePrimary=true`, no `[agent3b-theme-lock] off-theme` warning), and the judge's "ending contradicts the timeline" flag should clear. **Best batched with fixes #2/#3 so one run validates the structural triad.**

**Fix #2 (plant the culprit's unique-means discriminator before the reveal) LANDED (commit `ccc2585e`).** Targets the "reveal uses evidence not planted earlier" flag (fair-play 70/100). Prompt-primary + deterministic observability:
- **Agent 3 contract:** new HARD RULES — `9h` UNIQUE-MEANS discriminator (when the mechanism needs a special skill/tool/access, an early/mid reader-observable step must establish the culprit *uniquely* had it; never introduce it at the reveal) and `9i` PLANT-BEFORE-REVEAL (every `evidence_clue` must map to a scene strictly before `discriminating_test_scene`).
- **Agent 5:** plants an essential early/mid clue establishing the culprit's unique means/capability (elimination-type), alongside the existing trace/mechanism-link clue.
- **Detector:** `findUnplantedDiscriminatingClues` (pure, exported, 8 tests) finds evidence_clues unmapped or placed at/after the reveal scene; surfaced as a **non-fatal warning** in agent3-run (`[agent3-discriminating-planting]`) — promotable to a gate later.
- **Acceptance (pending live run):** no `[agent3-discriminating-planting]` warning; the reveal's discriminators are all planted earlier; `clues`/`ending` not regressed.

**Fix #3 (gate the mechanism reveal to the discriminating-test scene) LANDED (commit `7970092d`).** Targets the "mechanism explained too early" flag + pacing. Extends the existing `AGENT7_SCHEDULER_AUTHORITATIVE` flag (default OFF ⇒ unchanged):
- **Pure module `mechanism-reveal-gate.ts`** (6 tests): `resolveDiscriminatingSceneIndex` maps the CML `discriminating_test_scene` → global scene index; `stampMechanismRevealGate` marks pre-test scenes `mechanismRevealAllowed=false`, test scene onward `=true`. Best-effort, never throws.
- **agent7-run:** stamps the gate after the clue-job grid when the scheduler is authoritative (fills the scheduler's blind spot — it placed clue *reveals* but not the mechanism *explanation* beat).
- **agent9 prompt-builder:** a pre-test chapter gets a WITHHOLD instruction (observable clues only, keep the method open); the discriminating-test chapter gets a REVEAL-here instruction.
