...
| **1** | STOP THE VANDALISM: the parity path obeys the policy; no deterministic pronoun mutation under `verify` | ~1h | — | ✅ done — the ANALYSIS_44 "intentionally independent" gate now requires `checkingEnabled` (the abort it guarded against is warn-only today; rationale expired, documented in-code). Under `verify` NO deterministic pronoun mutation runs | 2026-07-23 |
