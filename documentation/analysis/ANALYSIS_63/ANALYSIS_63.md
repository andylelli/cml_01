...
**Board status as of 2026-07-22: THE BOARD IS CLOSED — 7.1–7.7 and 7.9 DONE; 7.8 exists only if the owner's optional M3 reads produce a gap > 3.** The 7.6 design AND build landed in one day (A_64); the owner cut the run budget 33% (one probe + M3 at exactly 4 — both spent, both clean); 7.9 closed 2026-07-22. **The batch era is over; the §2.8 corpus regime is in effect.**
...
| 7.9 | Close-out: data snapshot refresh · regime handoff | M3 batch (done) | 0 / £0 | — | ~30 min actual | ✅ **DONE 2026-07-22 — THE BATCH ERA IS OVER.** `campaign_data` snapshots refreshed with the `20260722` stamp (verified to contain the 7.2/7.3 batches, the A_64 probe, and all 4 M3 runs; 93-story corpus; the `20260720` originals untouched). Cleared en route: `standby-timeout-ac` ✅ 2700 · report dirs ✅ decided+deleted · skeleton-judge stays SHADOW. **The §2.8 regime is in effect: all future work derives from the preserved corpus; LLM spend is singular probes only; anything that seems to need a batch needs a better use of the corpus or a new owner decision.** Open carry-overs into the corpus era: the owner's optional M3 offset reads (`scratchpad/m3/read-packet.md` — the only remaining path to 7.8) · the judge-sensitivity question (do the near-verbatim complaint templates persist on planted-era stories?) · the craft-lever iteration toward 80 · pronoun-clarity candidate · mechanism-lever corpus question · voice generation-side design · injector deletion (last, alone) |
