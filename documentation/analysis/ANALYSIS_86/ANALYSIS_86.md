...
13. **Gender-agreement retry ("both women" in a paragraph with a man)**: route to the pronoun regen
    ...
    → **SUPERSEDED by three new items built 2026-09-10.** The reviewer-frequency analysis (their own "with X, this could reach" clauses across 12 reads) ranked the asks: chapter 10 (9 mentions), timing (16), reveal/ending (8), planting (4), scaffold (4), motive (3), weapon (2). Three of those had no lever at all and now do: `AGENT9_AFTERMATH_POSITIVE_JOB` (ch10 given a positive job — it had 4 requirements and 6 prohibitions and the model complied 0/22), `AGENT9_CLEARING_HUMAN_BEAT` (ch9 clearances must land a human beat, which is what separated the 85 from the ones asked to be trimmed), and `AGENT9_ONE_WEAPON` (11% of manuscripts put the killing evidence on two objects; reviewer-named twice as the headline issue). All three registered, tested and green.
14. **Final-reveal completeness**: the failure names two missing elements (motive, opportunity); the
    ...
    → **DONE as `AGENT3B_IMPLIED_DERIVATION` — and the premise moved once measured.** The plan was "make Agent 3b declare derivedFrom", but the prompt ALREADY requires it, with a worked example and a self-check, and is followed 17% of the time (31 of 180 facts). More prose will not move that. Instead the ONE unambiguous shape is inferred: a device holding exactly two clock facts and exactly one duration has a single arithmetic reading, so the duration is stamped with the inferred derivation and the EXISTING repair applies unchanged — rewrite, plausibility refusal, zero-length guard, read-back assertion, write-through to the device. Two durations or three clocks is ambiguous and refused. Reach: 9 devices have the shape undeclared, 8 disagree, 7 compute to a plausible interval. 6 tests.
15. **Retry with the failing paragraphs only in the negative example**, not the whole rejected draft.
    ...
    → **ALREADY BUILT 2026-08-17, with one positive data point.** The voice-fragment anti-verbatim guard exists on `characterVoiceSketches[].fragments` and carries the same analysis this item would have produced, citing the same probe. The four rows `probe:voice-fragment-leak` reports (17/28/39/44% conspicuous leakage against dialogue 7/7/6/6) are the PRE-guard baseline; the one read after it landed showed 0% and dialogue 8. n=1, so it is a lead, not a result — but there is nothing to build.
