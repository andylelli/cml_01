...
| **T1.7** | 8 | Wire the built `@cml/novelty` cross-run store | ◐ | ✓ | **constraints-feed core landed** — `apps/worker/.../novelty-ledger.ts` persists each shipped run's fingerprint and folds `recent(20)` into Agent 3's avoidance constraints; flag-gated `NOVELTY_CROSS_RUN` (default off); worker tsc + 5 tests green. The `judge`-based reject-and-regenerate gate + the T1.6 threshold flip are deferred as the authoritative escalation (see A_49 §1) |
...
**Planned in detail:** the four deferred items (T1.2/T1.3, T1.4, T1.6/T1.7, T1.8) now have code-grounded implementation plans — current state, phased steps, risks, flag-gating, and a live-run validation protocol — in **[ANALYSIS_49](../ANALYSIS_49/ANALYSIS_49.md)**.
