...
> **Status — constraints-feed core LANDED (flag-gated, default off).** Implemented steps 1-3 below as a worker-side module `apps/worker/src/jobs/novelty-ledger.ts` (kept `@cml/novelty` pure): persists each shipped run's avoidance fingerprint to `data/novelty-ledger.json` (atomic temp-write+rename, capped 500) and folds `recent(20)` into the Agent-3 avoidance constraints — the **zero-LLM divergence lever**. Wired into `mystery-orchestrator.ts` (load+merge after `buildNoveltyConstraints`; append-on-ship after status), all behind `NOVELTY_CROSS_RUN`. To avoid the fragile CML→enum mapping, the feed reuses the same string fields `buildNoveltyConstraints` reads from seeds rather than the strict `Fingerprint` enums. Worker `tsc` clean + 5 unit tests. **Deferred (the authoritative escalation):** step 4 (T1.6 threshold flip — affects all runs, adds LLM cost) and step 5 (the `judge`-based reject-and-regenerate gate — needs the enum `cmlToSkeleton` mapper). Next increment.
