...
- Agent 9 runtime tuning is centrally parameterized in `apps/worker/config/generation-params.yaml` (optional override: `CML_GENERATION_PARAMS_PATH`) for prose generation, anti-template linter thresholds, scoring-adapter semantic matching/fair-play window controls, and prose completeness scorer thresholds. The same YAML now includes planned scaffold sections for Agents 1-8 so all phase tunables can be centralized in one file as loader support is added.
