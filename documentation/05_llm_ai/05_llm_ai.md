...
**CML viability gate (implemented):** Before clue extraction, `@cml/cml` now hard-fails CML when `hidden_model.mechanism.description` or `discriminating_test.knowledge_revealed` is not grounded in reader-visible inference evidence, when `required_evidence` entries are abstract placeholders instead of concrete artifacts, witness statements, timestamps, traces, or access records, and when discriminating-test proof devolves into detective-only reaction shorthand such as `signals of guilt`, defensive reactions, or confession instead of factual pre-test evidence. These failures are routed back through Agent 4 revision instead of leaking into clue extraction.
...
**Retry resilience (implemented):** Agent 4 now continues bounded revision attempts after transient runtime failures (for example temporary LLM transport/service errors) instead of aborting immediately on first thrown exception.
...
**Deterministic contract parity (implemented):** Any clue set regenerated by Agent 6 now passes the same deterministic Agent 5 contract checks before reuse (source path legality, inference step bounds, cast-path consistency, audit consistency, era time style, locked-fact consistency, and discriminating evidence ID coverage).
**Source-path index resilience (implemented):** Agent 5 deterministic source-path repair now auto-clamps out-of-range array indexes for canonical CML paths (for example `CASE.constraint_space.access.actors[99]` -> last valid index) before hard-gating invalid source paths.
...
**Mechanism visibility contract (implemented):** Agent 5 first-pass requirements now explicitly demand one mechanism-visible early/mid clue and one culprit-named unique clue tied to the mechanism or preparation chain, rather than relying on inference-path wording alone.
**First-pass slot contract (implemented):** Agent 5 prompt now enforces three fixed essential early/mid clue slots in the initial response: `clue_mechanism_visibility_core`, `clue_core_contradiction_chain`, and `clue_core_elimination_chain`. This is prevention-first and intended to avoid retry-only recovery for fair-play timing failures.
**Mechanism visibility gate (implemented):** After clue extraction, Agent 5 now fails deterministically when no early/mid clue makes the core mechanism visible before the discriminating test.
...
**First-pass telemetry markers (implemented):** Agent 5 runtime now records first-pass KPI state in context (`agent5FirstPassPassed`, `agent5RetryInvoked`, `agent5FailureClass`) so canary boundary runs can emit deterministic first-pass status/retry/failure markers for rollups.
...
**Coverage freshness (implemented):** Agent 6 recomputes inference coverage from the current clue set after every clue regeneration before structural failure classification, preventing stale coverage snapshots from driving escalation.
**Coverage handoff consistency (implemented):** When Agent 6 recomputes coverage after clue regeneration, it now updates both `coverageResult` and `allCoverageIssues` from the same snapshot so downstream CML validation gates do not mix fresh coverage state with stale issue-classification data.
...
**Structural-revision context priority (implemented):** Agent 6 structural CML retry now derives cast names from the current CML first and only falls back to upstream cast artifacts if CML cast names are unavailable, preventing stale cast context from leaking into Agent 4 revision prompts.
**Pipeline hard stop (implemented):** Persistent critical failures after CML retry abort the pipeline with a descriptive error.
**Structural corroboration gate (implemented):** At the pre-prose CML validation gate, a standalone `Logical Deducibility` critical flag from Agent 6 is treated as structurally blocking only when corroborated by deterministic coverage/parity signals (for example critical clue coverage gaps, uncovered inference steps, or concurrent parity/no-withholding criticals). Uncorroborated standalone flags are downgraded to warnings to avoid false hard-stops from subjective audit variance.
**Cost circuit breaker (implemented):** Fair-play retry LLM cost is capped at $0.15 (MAX_FAIR_PLAY_RETRY_COST). Retry spend is accumulated across clue regenerations, re-audits, blind-reader remediation cycles, and structural revision retries. If the cap is exceeded, Agent 6 hard-stops immediately with an error.
**Retry cost accounting (implemented):** Agent 5 clue regeneration costs are accumulated across Agent 6 retries (instead of first-retry overwrite) so telemetry and retry budget accounting reflect total spend.
**Prevention-first feedback contract (implemented):** Agent 6 clue-regeneration feedback now carries strict first-pass acceptance statements (essential early/mid mechanism, contradiction, elimination; reader-solvable observation -> correction -> elimination ordering before Act III). Retries remain bounded safety, not the primary correctness mechanism.
**First-pass telemetry markers (implemented):** Agent 6 runtime now records first-pass KPI state in context (`agent6FirstPassPassed`, `agent6RetryInvoked`, `agent6FailureClass`) for canary boundary telemetry emission and run-summary KPI aggregation.
...
**Adaptive deterministic fill cap (implemented):** Post-retry deterministic clue anchoring now scales by outline size (25% of scene count, bounded to 3..8) instead of a fixed cap of 3, so medium/long outlines can recover larger residual pacing gaps without unnecessary hard failure.
...
### LLP prompt construction

LLP prompts are assembled as deterministic, priority-ordered blocks so each agent receives a stable contract instead of a free-form request.

How it is put together:
1. Start with immutable instruction layers: system constraints, agent role, and safety/fair-play rules.
2. Inject phase-local objective and required output contract (JSON/YAML shape, required fields, enum constraints, and no-new-facts boundary).
3. Attach normalized runtime context from upstream artifacts (setting, cast, CML, clues, outline, optional profile artifacts).
4. Add enforcement blocks: quality gates, chapter obligations (for prose), timeline/season locks, and discriminating-test obligations.
5. Add retry deltas when needed: only failed checks, corrective directives, and preserved immutable constraints.
6. Apply token budgeting and deterministic pruning to optional context while preserving critical constraints.

What it contains:
- Instruction header: role, hard constraints, allowed transformations.
- Task block: phase goal, completion criteria, and failure conditions.
- Schema/output block: exact response format and validation expectations.
- Context bundle: only relevant upstream artifacts and derived requirement lists.
- Guardrail block: fair-play rules, anti-template controls, setting/time locks, and no-hidden-evidence constraints.
- Retry feedback block (conditional): structured diagnostics from prior failed attempt.
- Final checklist: must-pass assertions before response generation.

This construction keeps prompt behavior reproducible across retries, limits drift between agents, and makes failures actionable by isolating which contract clause failed.
