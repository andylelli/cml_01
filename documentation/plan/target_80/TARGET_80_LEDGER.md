...
|---|---|---|
| **M1 — Floor** | 8/8 consecutive post-change runs reach a scored release gate (never aborted/failure) | **Attempt 3 in flight (count from `6dff6933`, 2026-07-15).** Attempt 1: 6/8 before the Agent 6.5 victim voice-sketch abort (`257f7855`). Attempt 2: 5/6 shipped, then the **dual-detective archetype** abort (`6dff6933`) — plus one mid-batch harness bug (canary exit read phase-threshold status; `64e5f49e`, count unaffected). Cumulative: **13 shipped runs, 3 aborts — every abort a distinct novel class, each fixed same-day.** The open question the gate must answer: is the abort-class pool finite? All 13 shipped runs feed the cap ledger and P4 pool. |
