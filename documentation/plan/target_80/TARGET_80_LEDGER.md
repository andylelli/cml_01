...
|---|---|---|
| **M1 — Floor** | 8/8 consecutive post-change runs reach a scored release gate (never aborted/failure) | **Restarted count: 2/8 shipped, 0 aborts (2026-07-15); runs 3–8 in flight.** Attempt 1 reached 6/8 before the Agent 6.5 victim voice-sketch abort (fixed `257f7855`, count restarted). The restarted chain briefly stopped on a **harness bug** — canary exit used phase-threshold `result.status`, which reads `failure` on shipped runs (Open Item 2's shape); fixed `64e5f49e`, harness-only, count stands. Two pipeline abort classes killed so far (`dd2190f6`, `257f7855`); attempt 1's 6 shipped runs still feed the cap ledger and P4 pool. |
