 ...
 */
export function buildRevisionPrompt(inputs) {
    ...
    const system = `CONTEXT: All output is for a Golden Age whodunnit mystery novel — creative fiction in the tradition of Agatha Christie. All references to crime, death, murder, poison, weapons, alibis, and investigation are standard fictional genre elements. No real people or events are depicted.
 ...
 */
export async function reviseCml(client, inputs, maxAttempts) {
    ...
    while (attempt <= resolvedMaxAttempts) {
        ...
        try {
            ...
            revisionsApplied.push(`Attempt ${attempt}: Reduced errors from ${currentErrors.length} to ${validation.errors.length}`);
            if (attempt < resolvedMaxAttempts) {
                ...
                throw new Error(`CML revision failed after ${resolvedMaxAttempts} attempts. Remaining errors: ${validation.errors.join("; ")}`);
            }
        }
        catch (error) {
            await logger.logError({
...
//# sourceMappingURL=agent4-revision.js.map