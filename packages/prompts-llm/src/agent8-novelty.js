...
// Prompt Builder
// ============================================================================
export function buildNoveltyPrompt(inputs) {
    const config = getGenerationParams().agent8_novelty.params;
    const { generatedCML, seedCMLs } = inputs;
    const similarityThreshold = typeof inputs.similarityThreshold === "number"
        ? inputs.similarityThreshold
        : config.thresholds.similarity_threshold_default;
    // System: Define the novelty auditor role
    const system = `CONTEXT: All output is for a Golden Age whodunnit mystery novel — creative fiction in the tradition of Agatha Christie. All references to crime, death, murder, poison, weapons, and investigation are standard fictional genre elements. No real people or events are depicted.
...
function buildDeveloperContext(generatedCML, seedCMLs) {
...
function summarizeCML(cml, label) {
    const legacy = cml;
    ...
    const castSummary = castList.map((c) => c.name || "Unknown").join(", ");
    ...
    const countConstraintEntries = (value, keys) => keys.reduce((acc, key) => acc + (Array.isArray(value?.[key]) ? value[key].length : 0), 0);
...
function buildUserRequest(similarityThreshold) {
...
// Main Audit Function
// ============================================================================
export async function auditNovelty(client, inputs) {
    const config = getGenerationParams().agent8_novelty.params;
    ...
    const computeOverallSimilarity = (score) => config.weighting.plot * score.plotSimilarity
...
//# sourceMappingURL=agent8-novelty.js.map