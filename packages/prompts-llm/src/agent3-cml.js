...
import { MYSTERY_EXPERT_SYSTEM, CML_SPECIALIST_SYSTEM, FAIR_PLAY_CHECKLIST, INFERENCE_PATH_QUALITY, } from "./shared/system.js";
import { buildEraConstraints, buildLocationConstraints, } from "./shared/constraints.js";
import { CML_2_0_SCHEMA_SUMMARY, AXIS_TYPE_DESCRIPTIONS, } from "./shared/schemas.js";
import { loadSeedCMLFiles, extractStructuralPatterns, selectRelevantPatterns, formatPatternsForPrompt, } from "./utils/seed-loader.js";
/**
 * Build the complete prompt for CML generation
 */
export function buildCMLPrompt(inputs, examplesDir) {
 ...
 */
export async function generateCML(client, inputs, examplesDir, maxAttempts) {
    ...
    const ensureString = (value, fallback) => typeof value === "string" && value.trim() ? value : fallback;
    const normalizeCml = (raw) => {
        ...
        const names = inputs.castNames?.length ? inputs.castNames : castArray.map((c) => c?.name).filter(Boolean);
        const normalizedCast = (names.length ? names : castArray.map((c) => c?.name).filter(Boolean)).map((name, index) => {
        ...
        if (!Array.isArray(inferencePath.steps) || inferencePath.steps.length < 3) {
            throw new Error("CML generation produced " + (Array.isArray(inferencePath.steps) ? inferencePath.steps.length : 0) +
        ...
        for (const step of inferencePath.steps) {
    ...
    let lastError;
    let lastValidation = { valid: false, errors: ["No attempts made"] };
            ...
            if (attempt < resolvedMaxAttempts) {
                ...
                try {
                    ...
                    await logger.logError({
                        ...
                        errorMessage: revisionError.message,
                        metadata: {
                            agent3Attempts: resolvedMaxAttempts,
                            originalErrors: validation.errors,
                        },
                    });
                    throw new Error(`CML generation failed after Agent 3 (${resolvedMaxAttempts} attempts) and Agent 4 revision. ` +
                        `Original errors: ${validation.errors.slice(0, 5).join("; ")}... ` +
                        `Revision error: ${revisionError.message}`);
                }
            }
        }
        catch (error) {
...
//# sourceMappingURL=agent3-cml.js.map