...
const generateVariationSeed = (runId) => {
 ...
 */
export function buildSettingPrompt(inputs) {
    // System prompt
    const system = `CONTEXT: All output is for a Golden Age whodunnit mystery novel — creative fiction in the tradition of Agatha Christie, Dorothy L. Sayers, and Arthur Conan Doyle. All references to crime, death, murder, poison, weapons, and investigation are standard fictional genre elements. No real people or events are depicted.
 ...
 */
export async function refineSetting(client, inputs, maxAttempts) {
        ...
        try {
            ...
            try {
                ...
                if (attempt < resolvedMaxAttempts) {
                    ...
                    throw new Error(`JSON parsing failed after ${resolvedMaxAttempts} attempts: ${parseError.message}`);
                }
            }
            // Validate structure
...
//# sourceMappingURL=agent1-setting.js.map