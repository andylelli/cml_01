 ...
 */
const deriveNameInitials = (hash, count) => {
...
const generateCastVariation = (runId, count) => {
...
// Prompt Building
// ============================================================================
export function buildCastPrompt(inputs) {
    const system = `CONTEXT: All output is for a Golden Age whodunnit mystery novel — creative fiction in the tradition of Agatha Christie and Dorothy L. Sayers. All references to crime, death, murder, poison, weapons, alibis, motives, and investigation are standard fictional genre elements. No real people or events are depicted.
...
// Main Agent Function
// ============================================================================
export async function designCast(client, inputs, maxAttempts) {
    ...
    const requiredUniqueArchetypes = Math.max(1, Math.ceil(expectedCount * config.quality.role_archetype.min_unique_ratio));
        ...
        try {
            ...
            try {
                ...
                if (attempt < resolvedMaxAttempts) {
                    ...
                    throw new Error(`JSON parsing failed after ${resolvedMaxAttempts} attempts: ${error.message}`);
                }
            }
            ...
            if (normalizedCharacters.length < expectedCount && attempt < resolvedMaxAttempts) {
                console.warn(`Attempt ${attempt}: Only ${normalizedCharacters.length} of ${expectedCount} characters returned. Retrying.`);
                continue;
            }
            if (normalizedCharacters.length !== expectedCount) {
                console.warn(`Attempt ${attempt}: Final attempt — expected ${expectedCount} characters, got ${normalizedCharacters.length}. Padding with placeholders.`);
            }
            ...
            if (missingFields.length > 0) {
                ...
                cast.characters = cast.characters.map((char, idx) => ({
                    ...
                    gender: (['male', 'female', 'non-binary'].includes(char.gender) ? char.gender : 'non-binary'),
            ...
            if (possibleCulprits.length < requiredCulprits) {
                if (attempt < resolvedMaxAttempts) {
                    console.warn(`Attempt ${attempt}: Only ${possibleCulprits.length} possibleCulprits, need ${requiredCulprits}. Retrying.`);
                    continue;
                }
            }
            // Validate role-archetype diversity: require >=70% unique labels.
            const uniqueArchetypes = new Set(cast.characters.map((char) => normalizeArchetypeKey(char.roleArchetype)).filter(Boolean));
            if (uniqueArchetypes.size < requiredUniqueArchetypes) {
                if (attempt < resolvedMaxAttempts) {
                    console.warn(`Attempt ${attempt}: Only ${uniqueArchetypes.size} unique role archetypes, need ${requiredUniqueArchetypes}. Retrying.`);
                    continue;
                }
                console.warn(`Attempt ${attempt}: Final attempt has ${uniqueArchetypes.size}/${requiredUniqueArchetypes} unique role archetypes. Applying deterministic diversification fallback.`);
                cast.characters = diversifyRoleArchetypes(cast.characters, requiredUniqueArchetypes);
                const uniqueAfterFallback = new Set(cast.characters.map((char) => normalizeArchetypeKey(char.roleArchetype)).filter(Boolean));
                if (uniqueAfterFallback.size < requiredUniqueArchetypes) {
                    throw new Error(`Cast role diversity guardrail failed after fallback (${uniqueAfterFallback.size}/${requiredUniqueArchetypes} unique archetypes).`);
                }
            }
            // Ensure every character has a gender declaration (fallback to non-binary)
            cast.characters = cast.characters.map((char) => ({
                ...char,
                gender: char.gender || 'non-binary',
            }));
            // Success!
...
//# sourceMappingURL=agent2-cast.js.map