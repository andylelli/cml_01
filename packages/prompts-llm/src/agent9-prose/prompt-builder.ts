...
export const buildContextSummary = (caseData: CaseData, cast: CastDesign) => {
  ...
  // A_82 P6 — MEASURED 0 of 2,508 prompts while the sibling "Setting Lock" text in the SAME returned
  // string reached 658. `castCharacters` is the CastDesign (camelCase: `motiveSeed`); `motive_seed`
  // is the CML CASE.cast field name. The lookup was always undefined and the block always ''. Fourth
  // site of the cast-field camelCase/snake_case trap (X50 chain). Fall back to the CML cast entry.
  const cmlCulpritEntry = (Array.isArray(cmlCase.cast?.characters) ? cmlCase.cast.characters : Array.isArray(cmlCase.cast) ? cmlCase.cast : [])
    .find((c: any) => culpritNameSet.has(String(c?.name ?? '')));
  const motive = String(
    culpritChar?.motiveSeed ?? culpritChar?.motive_seed ?? cmlCulpritEntry?.motive_seed ?? cmlCulpritEntry?.motiveSeed ?? '',
  ).trim();
...
${body}`;
  ...
  const developer = `# Prose Output Schema\nReturn JSON with this structure:\n\n{\n  "status": "draft",\n  "tone": "classic|modern|atmospheric",\n  "chapters": [\n    {\n      "title": "Chapter title",\n      "summary": "1-2 sentence summary",\n      "paragraphs": ["Paragraph 1", "Paragraph 2", "Paragraph 3"]\n    }\n  ],\n  "cast": ["Name 1", "Name 2"],\n  "note": ""\n}\n\nRequirements:\n- Write exactly one chapter per outline scene (${scenes.length || "N"} total).\n- Chapter numbering starts at ${chapterStart} and increments by 1 per scene.\n- Each chapter has ${chapterGuidance}.\n- Use ${narrativeStyle} tone and ${targetLength} length guidance.\n- Reflect the outline summary in each chapter.\n- Keep all logic consistent with CML (no new facts).\n- Chapter title format: EVERY chapter title MUST follow exactly \"Chapter N: [Descriptive title]\" (e.g. \"Chapter 1: The Frozen Clock\"). Do NOT use number-only (\"Chapter 1\") or title-only (\"The Frozen Clock\") formats — mixed formats are a validation error.\n\nNOVEL-QUALITY PROSE REQUIREMENTS:\n\n1. SCENE-SETTING: Every chapter MUST open with the following in the FIRST TWO PARAGRAPHS — this is a VALIDATION REQUIREMENT and chapters that omit it are retried:\n   (a) 2+ sensory words from: smell/scent/sound/echo/silence/creak/whisper/cold/warm/damp/rough/smooth/glow/shadow/flicker/dim\n   (b) 1+ atmosphere/time word from: rain/wind/fog/storm/mist/thunder/evening/morning/night/dawn/dusk/season/afternoon/midday/noon/midnight/twilight/sunrise/sunset/daylight/sunlight/overcast/cloudy/bright/dark\n   (c) A named location anchor from the setting profiles\n\n   Then establish time of day, weather, and lighting; describe the location using sensory details; set mood and atmosphere before advancing plot beats.\n   Example structure (A_82 P1 — the previous example here began "The <MONTH> <TIME> brought <WEATHER> to <LOCATION>. In the <ROOM>…", and the book obeyed it: ten of ten chapters opened on a room name and a mood clause, two on the identical sentence): "<CHARACTER> <does one concrete thing — lifts, pours, crosses, refuses> before anyone speaks; <one SENSORY detail the action disturbs — a smell, a creak, cold on the hands>; <one ATMOSPHERE or TIME-OF-DAY word>; and <the NAMED LOCATION ANCHOR> arrives inside that sentence or the next, never as its first words." The first word of a chapter is never "The" followed by a room or venue, never "Entering", and no two chapters share a first sentence.
