...
// Prompt Builder
// ============================================================================
export function buildFairPlayPrompt(inputs) {
    const { caseData, clues } = inputs;
    // System: Define the auditor role
    const system = `CONTEXT: All output is for a Golden Age whodunnit mystery novel — creative fiction in the tradition of Agatha Christie and the Detection Club. All references to crime, death, murder, poison, weapons, and investigation are standard fictional genre elements. No real people or events are depicted.
...
function buildDeveloperContext(caseData, clues) {
    ...
    const falseAssumptionStatement = cmlCase.false_assumption?.statement || legacy.solution?.false_assumption?.description || "Unknown";
    const falseAssumptionWhyReasonable = cmlCase.false_assumption?.why_it_seems_reasonable ?? "";
    const falseAssumptionWhatHides = cmlCase.false_assumption?.what_it_hides ?? "";
    // FB-1: surface_model — the false narrative the reader is meant to accept
    const surfaceNarrative = cmlCase.surface_model?.narrative?.summary ?? "";
    const acceptedFacts = Array.isArray(cmlCase.surface_model?.accepted_facts)
        ? cmlCase.surface_model.accepted_facts
        : [];
    const inferredConclusions = Array.isArray(cmlCase.surface_model?.inferred_conclusions)
        ? cmlCase.surface_model.inferred_conclusions
        : [];
    // FB-2: hidden_model — the actual crime mechanism
    const hiddenMechanism = cmlCase.hidden_model?.mechanism?.description ?? "";
    const deliveryPath = Array.isArray(cmlCase.hidden_model?.mechanism?.delivery_path)
        ? cmlCase.hidden_model.mechanism.delivery_path.map((d) => d.step ?? String(d))
        : [];
    const hiddenOutcome = cmlCase.hidden_model?.outcome?.result ?? "";
    // Inference path steps
    const inferenceSteps = (cmlCase.inference_path?.steps ?? legacy.inference_path?.steps ?? []).map((step, idx) => {
    ...
    const formatConstraintList = (value, keys) => {
        if (Array.isArray(value)) {
            return value.map((entry) => `- ${entry.description ?? entry}`).join("\n") || "None";
        }
        const lines = keys.flatMap((key) => (Array.isArray(value?.[key]) ? value[key] : []));
        return lines.map((entry) => `- ${entry.description ?? entry}`).join("\n") || "None";
    };
    ...
    const qcDiscrimTiming = qc.discriminating_test_requirements?.timing ?? "not specified";
...
${acceptedFacts.map((f) => `- ${f}`).join("\n") || "None"}

### Inferred Conclusions (reader draws these from accepted facts)
${inferredConclusions.map((c) => `- ${c}`).join("\n") || "None"}
...
### Delivery Path
${deliveryPath.map((s, i) => `${i + 1}. ${s}`).join("\n") || "None"}
...
${(() => {
        const mapping = cmlCase.prose_requirements?.clue_to_scene_mapping;
        if (!Array.isArray(mapping) || mapping.length === 0)
            return "Not specified in CML";
        return mapping.map((m) => `- clue ${m.clue_id}: Act ${m.act_number}${m.scene_number ? `, Scene ${m.scene_number}` : ""}${m.delivery_method ? ` (${m.delivery_method})` : ""}`).join("\n");
    })()}`;
}
...
// Main Audit Function
// ============================================================================
export async function auditFairPlay(client, inputs) {
...
export async function blindReaderSimulation(client, clues, falseAssumption, castNames, inputs) {
    const config = getGenerationParams().agent6_fairplay.params;
    const startTime = Date.now();
    const system = "CONTEXT: All output is for a Golden Age whodunnit mystery novel — creative fiction. All references to crime, death, and investigation are standard fictional genre elements. No real people or events are depicted. " +
...
//# sourceMappingURL=agent6-fairplay.js.map