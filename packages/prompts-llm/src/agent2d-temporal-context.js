...
const generateSpecificDate = (decade, runId) => {
    const monthNames = [
...
const buildTemporalContextPrompt = (inputs, previousErrors) => {
    ...
    const system = `CONTEXT: All output is for a Golden Age whodunnit mystery novel — creative fiction in the tradition of Agatha Christie. All references to crime, death, murder, and investigation are standard fictional genre elements. No real people or events are depicted.
...
Make this mystery feel ROOTED in a specific moment in time, not just a generic era.${inputs.qualityGuardrails && inputs.qualityGuardrails.length > 0
...
export async function generateTemporalContext(client, inputs, maxAttempts) {
    ...
    if (retryResult.validationResult.warnings && retryResult.validationResult.warnings.length > 0) {
        console.warn(`[Agent 2d] Temporal context validation warnings:\n` +
    ...
    if (!retryResult.validationResult.valid) {
        console.error(`[Agent 2d] Temporal context failed validation after ${resolvedMaxAttempts} attempts:\n` +
...
//# sourceMappingURL=agent2d-temporal-context.js.map