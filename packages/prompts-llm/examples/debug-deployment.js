...
async function testDirectCall() {
    ...
    const client = new OpenAIClient(endpoint, new AzureKeyCredential(apiKey), { apiVersion: "2024-02-15-preview" });
    try {
        console.log("Test 1: Simple call (5 tokens)...");
        const result1 = await client.getChatCompletions(deployment, [{ role: "user", content: "Say hi" }], { maxTokens: 5 });
        console.log(`✅ Success: ${result1.choices[0]?.message?.content}\n`);
        console.log("Test 2: With system message (100 tokens)...");
        const result2 = await client.getChatCompletions(deployment, [
        ...
        const result3 = await client.getChatCompletions(deployment, [
        ...
        const result4 = await client.getChatCompletions(deployment, [
            { role: "system", content: "You are a helpful assistant." },
        ...
        const result5 = await client.getChatCompletions(deployment, [
            { role: "system", content: "You are a helpful assistant." },
...
testDirectCall();
//# sourceMappingURL=debug-deployment.js.map