...
import { designCast } from "../src/agent2-cast.js";
const makeInputs = () => ({
...
const makeCharacter = (name, roleArchetype) => ({
    name,
    ageRange: "40s",
    occupation: "resident",
    roleArchetype,
...
describe("Agent 2 cast diversity hardening", () => {
    ...
    it("fails final attempt when fallback cannot recover diversity", async () => {
        ...
        const client = { chat };
        await expect(designCast(client, makeInputs(), 1)).rejects.toThrow(/role diversity guardrail failed after fallback/i);
        expect(chat).toHaveBeenCalledTimes(1);
    });
});
//# sourceMappingURL=agent2-cast-diversity.test.js.map