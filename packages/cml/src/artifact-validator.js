...
const loadArtifactSchema = (artifactType) => {
    if (schemaCache.has(artifactType)) {
        return schemaCache.get(artifactType);
    }
    const currentDir = path.dirname(fileURLToPath(import.meta.url));
    const schemaPath = path.resolve(currentDir, "..", "..", "..", "schema", `${artifactType}.schema.yaml`);
    try {
...
const validateNode = (node, value, pathLabel, errors, warnings) => {
 ...
 */
export const validateArtifact = (artifactType, payload) => {
    try {
 ...
 */
export const validateArtifacts = (artifacts) => {
    const results = {};
