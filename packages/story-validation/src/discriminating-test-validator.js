...
function findComponentInScene(sceneNumber, text, pattern) {
...
function toCoverage(sceneNumber, text) {
...
function missingComponentToError(missingComponent, fallbackLocation) {
...
export class DiscriminatingTestValidator {
    name = 'DiscriminatingTestValidator';
    llmClient;
    constructor(llmClient) {
        this.llmClient = llmClient;
    }
    async validate(story, cml) {
        const errors = [];
        ...
        const validTestScenes = sceneCoverage.filter((scene) => scene.setup && scene.evidence && scene.elimination && scene.outcome);
        ...
        if (!hasValidTest) {
            ...
            const fallbackLocation = strongestCandidate?.setup ??
            ...
            if (strongestCandidate && strongestCandidate.score > 0) {
                ...
                if (!strongestCandidate.setup)
                    missingComponents.push('setup');
                if (!strongestCandidate.evidence)
                    missingComponents.push('evidence');
                if (!strongestCandidate.elimination)
                    missingComponents.push('elimination');
                if (!strongestCandidate.outcome)
                    missingComponents.push('outcome');
...
//# sourceMappingURL=discriminating-test-validator.js.map