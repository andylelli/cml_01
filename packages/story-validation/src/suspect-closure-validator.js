...
export class SuspectClosureValidator {
            ...
            if (!hasValidElimination && this.llmClient) {
                ...
                for (const scene of story.scenes) {
                    if (scene.text?.includes(suspect)) {
                        const semanticResult = await semanticValidateSuspectElimination(scene.text || '', suspect, this.llmClient);
            ...
            if (!hasValidEvidence && this.llmClient) {
                ...
                for (const scene of story.scenes) {
                    if (scene.text?.includes(culprit)) {
                        const semanticResult = await semanticValidateCulpritEvidence(scene.text || '', culprit, this.llmClient);
                        if (semanticResult.isValid && semanticResult.confidence !== 'low') {
...
//# sourceMappingURL=suspect-closure-validator.js.map