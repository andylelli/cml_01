 ...
 */
function genderToPronouns(gender) {
...
function oppositePronouns(gender) {
    switch (gender.toLowerCase()) {
        case 'male':
            return { subject: 'she', object: 'her', possessive: 'her' };
        case 'female':
...
export class ProseConsistencyValidator {
    ...
    // ---------------------------------------------------------------------------
    checkLockedFacts(story, cml) {
        const lockedFacts = cml?.lockedFacts ?? [];
     ...
     */
    detectConflictingValue(text, canonicalValue) {
    ...
    checkPronounDrift(story, cml) {
        ...
        for (const castMember of cml.CASE.cast) {
            ...
            if (!gender || gender === 'non-binary')
                continue; // non-binary uses they/them; no binary drift to detect
    ...
    checkOpeningStyles(story) {
        if (story.scenes.length < 4)
            return []; // too few chapters to penalise repetition
        const styleCounts = new Map(); // style → scene numbers
        for (const scene of story.scenes) {
...
//# sourceMappingURL=prose-consistency-validator.js.map