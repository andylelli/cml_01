...
export class TemporalContextScorer {
    ...
    validateStructure(output) {
        const tests = [];
        if (!output) {
            tests.push(fail('Output exists', 'validation', 3.0, 'Temporal context output is missing', 'critical'));
            return tests;
        }
        // Required fields
        ...
        for (const field of requiredFields) {
            if (!exists(output[field])) {
                tests.push(fail(`${field} present`, 'validation', 1.0, `Missing required field: ${field}`, 'major'));
            }
            else {
        ...
        if (output.specific_date) {
            ...
            tests.push(dateSpecificity.score >= 70
        ...
        if (output.time_of_day) {
            ...
            tests.push(timeSpecificity.score >= 70
    ...
    checkCompleteness(output) {
        ...
        tests.push(substantialFields >= 4
        ...
        tests.push(optionalRichnessScore >= 70
    ...
    hasSubstantialSpecificDate(value) {
    ...
    hasSubstantialSeason(value) {
        if (typeof value !== 'string' || value.trim().length === 0)
            return false;
        return /(spring|summer|fall|autumn|winter)/i.test(value);
    }
    hasSubstantialWeather(value) {
        if (typeof value !== 'string' || value.trim().length === 0)
            return false;
        const wordCount = value.split(/\s+/).filter(Boolean).length;
        return wordCount >= 8;
    }
    scoreOptionalRichness(output) {
    ...
    checkConsistency(output, context) {
        ...
        if (output.season && output.weather_patterns) {
            const consistent = this.checkSeasonWeatherConsistency(output.season, output.weather_patterns);
            tests.push(consistent
        ...
        if (output.fashion_and_attire && context.cml) {
            const era = context.cml?.BACKGROUND_CONTEXT?.era_background?.decade;
            if (era) {
                const fashionEraConsistent = this.checkFashionEraConsistency(output.fashion_and_attire, era);
                tests.push(fashionEraConsistent
        ...
        if (output.specific_date && context.cml) {
            const era = context.cml?.BACKGROUND_CONTEXT?.era_background?.decade;
            if (era) {
                const dateEraConsistent = this.checkDateEraConsistency(output.specific_date, era);
                tests.push(dateEraConsistent
    ...
    checkSeasonWeatherConsistency(season, weather) {
    ...
    checkFashionEraConsistency(fashion, era) {
...
//# sourceMappingURL=agent2d-temporal-context-scorer.js.map