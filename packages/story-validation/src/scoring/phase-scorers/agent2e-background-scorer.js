import { createTest, pass, fail, partial, exists, calculateCategoryScore, getCriticalFailures, } from '../scorer-utils.js';
/**
...
export class BackgroundContextScorer {
    ...
    validateStructure(output) {
        ...
        if (!output) {
            tests.push(fail('Output exists', 'validation', 2.0, 'Background context output is null or undefined', 'critical'));
            return tests;
        }
        ...
        if (output.era_background) {
            ...
            tests.push(exists(era.tech_capabilities)
            ...
            tests.push(exists(era.forensic_capabilities)
        ...
        if (output.location_background) {
            ...
            tests.push(fail('Location background exists', 'validation', 2.0, 'Location background section is missing', 'critical'));
        }
        // Constraints section
        if (output.constraints) {
            ...
            tests.push(partial('Constraints section present', 'validation', 50, 0.5, 'Constraints section recommended but not critical', 'minor'));
        }
        return tests;
    }
    /**
    ...
    checkCompleteness(output) {
        ...
        const eraScore = output.era_background
            ...
            : 0;
        tests.push(createTest('Era background complete', 'completeness', eraScore >= 70, eraScore, 2.0, `Era completeness: ${eraScore}%`));
        ...
        const locScore = output.location_background
            ...
            : 0;
        tests.push(createTest('Location background complete', 'completeness', locScore >= 70, locScore, 2.0, `Location completeness: ${locScore}%`));
        ...
        if (output.constraints) {
            ...
            tests.push(createTest('Constraints defined', 'completeness', constraintsScore >= 50, constraintsScore, 1.0, `Constraints completeness: ${constraintsScore}%`));
        }
        else {
            tests.push(partial('Constraints defined', 'completeness', 30, 1.0, 'Constraints section missing (optional)'));
        }
        return tests;
    }
    /**
     ...
     */
    scoreEraCompleteness(era) {
        let score = 0;
        let maxScore = 0;
        // Decade (required)
        maxScore += 20;
        if (exists(era.decade))
            score += 20;
        // Tech capabilities (required)
        maxScore += 20;
        if ((era.tech_capabilities?.length || 0) >= 3)
            score += 20;
        else
            score += (era.tech_capabilities?.length || 0) * 6;
        // Forensic capabilities (required)
        maxScore += 20;
        if ((era.forensic_capabilities?.length || 0) >= 3)
            score += 20;
        else
            score += (era.forensic_capabilities?.length || 0) * 6;
        // Communications (important)
        maxScore += 15;
        if ((era.communications?.length || 0) >= 2)
            score += 15;
        else
            score += (era.communications?.length || 0) * 7;
        // Transportation (important)
        maxScore += 15;
        if ((era.transportation?.length || 0) >= 2)
            score += 15;
        else
            score += (era.transportation?.length || 0) * 7;
        // Social norms (nice to have)
        maxScore += 10;
        if ((era.social_norms?.length || 0) >= 2)
            score += 10;
        else
            score += (era.social_norms?.length || 0) * 5;
        return Math.round((score / maxScore) * 100);
    }
    /**
     * Score location background completeness (0-100)
     */
    scoreLocationCompleteness(loc) {
        let score = 0;
        let maxScore = 0;
        // Location type (required)
        maxScore += 25;
        if (exists(loc.location_type))
            score += 25;
        // Physical constraints (required)
        maxScore += 25;
        if ((loc.physical_constraints?.length || 0) >= 3)
            score += 25;
        else
            score += (loc.physical_constraints?.length || 0) * 8;
        // Security features (important)
        maxScore += 20;
        if ((loc.security_features?.length || 0) >= 2)
            score += 20;
        else
            score += (loc.security_features?.length || 0) * 10;
        // Accessibility (important)
        maxScore += 15;
        if ((loc.accessibility?.length || 0) >= 2)
            score += 15;
        else
            score += (loc.accessibility?.length || 0) * 7;
        // Atmosphere (nice to have)
        maxScore += 15;
        if ((loc.atmosphere?.length || 0) >= 2)
            score += 15;
        else
            score += (loc.atmosphere?.length || 0) * 7;
        return Math.round((score / maxScore) * 100);
    }
    /**
    ...
    checkConsistency(output, context) {
        ...
        if (output.era_background && output.location_background) {
            tests.push(pass('Era/Location compatibility', 'consistency', 1.0, 'Era and location background both present'));
        }
        else {
            tests.push(fail('Era/Location compatibility', 'consistency', 1.0, 'Missing era or location background', 'major'));
        }
        ...
        if (output.constraints && output.era_background) {
            ...
            const conflicts = anachronisms.filter(item => techCapabilities.some(tech => tech.toLowerCase().includes(item.toLowerCase())));
            if (conflicts.length > 0) {
                tests.push(fail('Constraint/Capability consistency', 'consistency', 1.0, `Anachronisms found in capabilities: ${conflicts.join(', ')}`, 'major'));
            }
            else {
                tests.push(pass('Constraint/Capability consistency', 'consistency', 1.0, 'No anachronism conflicts detected'));
            }
        }
        return tests;
    }
...
//# sourceMappingURL=agent2e-background-scorer.js.map