...
export class HardLogicScorer {
    ...
    validateStructure(output) {
        ...
        if (!output || !Array.isArray(output.hard_logic_devices)) {
            tests.push(fail('Devices array exists', 'validation', 3.0, 'Hard logic devices array is missing or not an array', 'critical'));
            return tests;
        }
    ...
    validateDevice(device) {
        ...
        // Required fields
        if (!exists(device.id)) {
            tests.push(fail(`${deviceId}: has ID`, 'validation', 0.5, 'Device missing ID', 'major'));
        }
        else {
            tests.push(pass(`${deviceId}: has ID`, 'validation', 0.1));
        }
        if (!exists(device.type)) {
            tests.push(fail(`${deviceId}: has type`, 'validation', 0.5, 'Device missing type', 'major'));
        }
        else {
            tests.push(pass(`${deviceId}: has type`, 'validation', 0.1));
        }
        if (!exists(device.why_necessary)) {
            tests.push(fail(`${deviceId}: has justification`, 'validation', 0.5, 'Device missing why_necessary field', 'major'));
        }
        else {
    ...
    assessQuality(output) {
        ...
        const withImplications = output.hard_logic_devices.filter(d => d.implications && d.implications.length > 0).length;
        tests.push(withImplications >= output.hard_logic_devices.length * 0.7
    ...
    scoreDeviceDepth(device) {
        ...
        if (device.description && device.description.length > 30)
            score += 20;
        else if (exists(device.description))
            score += 10;
        // Has why_necessary (should be detailed)
        if (device.why_necessary && device.why_necessary.length > 30)
            score += 20;
        else if (exists(device.why_necessary))
            score += 10;
        // Has implications
        if ((device.implications?.length || 0) >= 2)
            score += 15;
        else if ((device.implications?.length || 0) >= 1)
            score += 7;
        // Has red herring potential
        if (exists(device.red_herring_potential))
            score += 15;
    ...
    checkCompleteness(output, input) {
        ...
        const types = new Set(output.hard_logic_devices.map(d => d.type).filter(exists));
        tests.push(types.size >= Math.min(deviceCount, 3)
    ...
    checkConsistency(output, context) {
        ...
        tests.push(uniqueIds.size === ids.length
...
//# sourceMappingURL=agent4-hard-logic-scorer.js.map