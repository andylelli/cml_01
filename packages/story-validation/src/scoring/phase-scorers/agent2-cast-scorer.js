import { pass, fail, partial, exists, calculateCategoryScore, scoreArrayCompleteness, checkDuplicates, getCriticalFailures, } from '../scorer-utils.js';
/**
...
export class CastDesignScorer {
    ...
    validateStructure(output) {
        ...
        if (!output || !Array.isArray(output.cast)) {
            tests.push(fail('Cast array exists', 'validation', 3.0, 'Cast array is missing or not an array', 'critical'));
            return tests;
        }
        tests.push(pass('Cast array exists', 'validation', 1.0));
        // Check cast not empty
        if (output.cast.length === 0) {
            tests.push(fail('Cast not empty', 'validation', 3.0, 'Cast array is empty', 'critical'));
            return tests;
        }
        ...
        if (validRate < 0.7) {
            tests.push(fail('Character validity rate', 'validation', 2.0, `Only ${Math.round(validRate * 100)}% of characters are valid (need 70%)`, 'major'));
        }
        else if (validRate < 1.0) {
            tests.push(partial('Character validity rate', 'validation', Math.round(validRate * 100), 2.0, `${Math.round(validRate * 100)}% of characters are valid`));
        }
        else {
            tests.push(pass('Character validity rate', 'validation', 2.0, 'All characters valid'));
        }
    ...
    validateCharacter(char) {
        ...
        for (const field of requiredFields) {
            if (!exists(char[field])) {
                tests.push(fail(`${charName}: ${field}`, 'validation', 0.5, `Missing required field: ${field}`, 'major'));
            }
            else {
                tests.push(pass(`${charName}: ${field}`, 'validation', 0.1));
            }
        }
        return tests;
    }
     ...
     */
    scoreCharacterDepth(char) {
        let score = 0;
        // Has name (required)
        if (exists(char.name))
            score += 10;
        // Has role archetype
        if (exists(char.role_archetype))
            score += 10;
        // Has public persona (should be descriptive)
        if (char.public_persona && char.public_persona.length > 20)
            score += 15;
        else if (exists(char.public_persona))
            score += 5;
        // Has private secret (should be descriptive)
        if (char.private_secret && char.private_secret.length > 20)
            score += 15;
        else if (exists(char.private_secret))
            score += 5;
        // Has motive
        if (char.motive_seed && char.motive_seed.length > 15)
            score += 15;
        else if (exists(char.motive_seed))
            score += 5;
        // Has relationships
        if ((char.relationships?.length || 0) >= 2)
            score += 10;
        else if ((char.relationships?.length || 0) >= 1)
            score += 5;
        // Has opportunity channels
        if ((char.opportunity_channels?.length || 0) >= 2)
            score += 10;
        else if ((char.opportunity_channels?.length || 0) >= 1)
            score += 5;
        // Has behavioral tells
        if ((char.behavioral_tells?.length || 0) >= 2)
            score += 10;
        else if ((char.behavioral_tells?.length || 0) >= 1)
            score += 5;
        // Has stakes
        if (exists(char.stakes))
            score += 10;
     ...
     */
    assessRelationshipQuality(cast) {
        const tests = [];
        // Count total relationships
        const totalRelationships = cast.reduce((sum, char) => sum + (char.relationships?.length || 0), 0);
        const avgRelationships = totalRelationships / cast.length;
        tests.push(avgRelationships >= 2
     ...
     */
    checkCompleteness(output, input) {
        const tests = [];
        // Check cast size
        const castSize = output.cast?.length || 0;
        const targetSize = input.cast_size || 8;
        tests.push(scoreArrayCompleteness(output.cast, Math.max(4, targetSize - 2), // Min is target-2, but at least 4
        targetSize, 'Cast size appropriate', 'completeness'));
        // Check that we have at least one culprit-eligible character
        const eligibleCount = output.cast.filter(c => c.culprit_eligibility === 'eligible').length;
        tests.push(eligibleCount >= 3
        ...
        const archetypes = new Set(output.cast.map(c => c.role_archetype).filter(exists));
        const diversityScore = Math.min(100, (archetypes.size / castSize) * 100);
        tests.push(archetypes.size >= castSize * 0.7
    ...
    checkCulpritEligibility(cast) {
        ...
        if (locked.length > 0) {
            tests.push(lockedWithCulpability.length === locked.length
     ...
     * Check relationship reciprocity
     */
    checkRelationshipReciprocity(cast) {
        const tests = [];
            ...
            for (const rel of char.relationships) {
                ...
                for (const [name, otherChar] of nameMap) {
                    ...
                    if (rel.toLowerCase().includes(name)) {
                        ...
                        if (otherChar.relationships?.some(r => r.toLowerCase().includes(char.name.toLowerCase()))) {
...
//# sourceMappingURL=agent2-cast-scorer.js.map