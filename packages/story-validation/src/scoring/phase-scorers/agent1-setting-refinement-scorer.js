...
export class SettingRefinementScorer {
    ...
    validateStructure(output) {
        const tests = [];
        if (!output) {
            tests.push(fail('Output exists', 'validation', 3.0, 'Setting refinement output is missing', 'critical'));
            return tests;
        }
        if (!Array.isArray(output.locations)) {
            tests.push(fail('Locations array exists', 'validation', 2.0, 'Locations array is missing', 'critical'));
            return tests;
        }
        tests.push(pass('Locations array exists', 'validation', 0.5));
        if (output.locations.length === 0) {
            tests.push(fail('Locations not empty', 'validation', 2.0, 'No locations defined', 'critical'));
            return tests;
        }
    ...
    assessQuality(output) {
        ...
        tests.push(plausibleLocations >= output.locations.length * 0.8
        ...
        tests.push(avgLayoutQuality >= 70
        ...
        if (cluePlacements.length > 0) {
            tests.push(wellJustified >= cluePlacements.length * 0.7
    ...
    scoreLayoutDetail(location) {
        ...
        if (location.key_features && Array.isArray(location.key_features)) {
            ...
            const detailedFeatures = location.key_features.filter((f) => f.split(/\s+/).length >= 3).length;
            if (detailedFeatures >= 3)
                score += 20;
            else if (detailedFeatures >= 1)
                score += 10;
        }
    ...
    checkCompleteness(output, context) {
        ...
        if (context.previous_phases && context.previous_phases.agent2c_location_profiles) {
            const profiles = context.previous_phases.agent2c_location_profiles?.location_profiles || [];
            const profileNames = profiles.map((p) => p.location_name?.toLowerCase()).filter((n) => !!n);
            const refinementNames = output.locations?.map(l => l.name?.toLowerCase()).filter((n) => !!n) || [];
        ...
        tests.push(exists(output.physical_constraints)
        ...
        tests.push(exists(output.accessibility_notes)
    ...
    checkConsistency(output, context) {
        ...
        const locationNames = output.locations.map(l => l.name).filter((name) => exists(name));
        const uniqueNames = new Set(locationNames.map(n => n.toLowerCase()));
        tests.push(uniqueNames.size === locationNames.length
    ...
    clueInCML(placementClue, cmlClues) {
...
//# sourceMappingURL=agent1-setting-refinement-scorer.js.map