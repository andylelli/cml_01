...
export class CharacterProfilesScorer {
    ...
    validateStructure(output) {
        ...
        if (!output || !Array.isArray(output.character_profiles)) {
            tests.push(fail('Profiles array exists', 'validation', 3.0, 'Character profiles array is missing', 'critical'));
            return tests;
        }
        tests.push(pass('Profiles array exists', 'validation', 1.0));
        if (output.character_profiles.length === 0) {
            tests.push(fail('Profiles not empty', 'validation', 3.0, 'No character profiles generated', 'critical'));
            return tests;
        }
    ...
    validateProfile(profile) {
        ...
        for (const field of requiredFields) {
            if (!exists(profile[field])) {
                tests.push(fail(`${name}: ${field}`, 'validation', 0.5, `Missing required field: ${field}`, 'major'));
            }
            else {
    ...
    assessQuality(output) {
        ...
        const withHumour = output.character_profiles.filter(p => exists(p.humour_style) && p.humour_style.length > 20).length;
        tests.push(withHumour >= output.character_profiles.length * 0.7
    ...
    scoreProfileDepth(profile) {
        ...
        // Behavioral tells
        if (exists(profile.behavioral_tells))
            score += 10;
        // Humour style
        if (exists(profile.humour_style))
            score += 5;
        return Math.min(100, score);
    }
    assessPersonalityDistinction(profiles) {
        const tests = [];
        ...
        if (personas.length >= 2) {
            ...
            const distinctCount = new Set(personas.map(p => p.substring(0, 50).toLowerCase())).size;
            const distinctRate = distinctCount / personas.length;
            tests.push(distinctRate >= 0.8
    ...
    checkCompleteness(output, input) {
        ...
        tests.push(avgFieldCompleteness >= 80
    ...
    checkConsistency(output, context) {
        ...
        if (context.previous_phases && context.cml) {
            ...
            const castNames = cast.map((c) => c.name?.toLowerCase()).filter((n) => n);
            let matchingNames = 0;
            for (const profile of output.character_profiles) {
                const profileName = (profile.character_name || '').toLowerCase();
                if (castNames.some((cn) => profileName.includes(cn) || cn.includes(profileName))) {
                    matchingNames++;
                }
            }
...
//# sourceMappingURL=agent2b-character-profiles-scorer.js.map