...
import { calculateGrade, passesThreshold, getFailedComponents, getThreshold, COMPONENT_MINIMUMS, DEFAULT_THRESHOLDS, } from "../thresholds.js";
...
function makeScore(overrides = {}, agent = "agent2-cast") {
    return {
        agent,
        validation_score: 80,
        quality_score: 75,
...
const standardConfig = { mode: "standard" };
const strictConfig = { mode: "strict" };
const lenientConfig = { mode: "lenient" };
// ─── Grade Calculation ────────────────────────────────────────────────────────
...
describe("getThreshold", () => {
    ...
    it("uses override when provided", () => {
        const cfg = { mode: "standard", overrides: { "agent2-cast": 90 } };
...
describe("ScoreAggregator", () => {
    ...
    it("marks overall report as passed only when all phases pass", () => {
        const agg = new ScoreAggregator(standardConfig);
        agg.addPhaseScore("agent2-cast", "Cast Design", makeScore({ total: 80, passed: true }), 1000);
        agg.addPhaseScore("agent4-hard-logic", "Hard Logic", makeScore({ agent: "agent4-hard-logic", total: 60, passed: false }), 2000);
        const report = agg.generateReport(metadata);
    ...
    it("marks report passed when all phases pass", () => {
        ...
        agg.addPhaseScore("agent7-narrative", "Narrative", makeScore({ agent: "agent7-narrative", total: 80 }), 1500);
        const report = agg.generateReport(metadata);
    ...
    it("includes all added phases in the report", () => {
        ...
        agg.addPhaseScore("agent4-hard-logic", "Hard Logic", makeScore({ agent: "agent4-hard-logic" }), 2000);
        const report = agg.generateReport(metadata);
    ...
    it("calculates summary stats: phases_passed and phases_failed", () => {
        ...
        agg.addPhaseScore("agent4-hard-logic", "Hard Logic", makeScore({ agent: "agent4-hard-logic", total: 50, passed: false, validation_score: 40 }), 2000);
        const report = agg.generateReport(metadata);
    ...
    it("computes stage-aware validation reconciliation from release gate snapshots", () => {
        ...
        agg.upsertDiagnostic("agent9_prose_release_gate_summary", "agent9_prose", "Release Gate", "release_gate_summary", {
