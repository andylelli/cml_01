...
export class StoryValidationPipeline {
    ...
    generateReport(errors, cml, failedAt) {
        const summary = {
    ...
    buildConsistencyReport(errors, cml) {
        const lockedFactsChecked = cml?.lockedFacts?.length ?? 0;
        const lockedFactsViolations = errors.filter(e => e.type === 'locked_fact_contradicted' || e.type === 'locked_fact_missing_value').length;
        ...
        const contextLeakageViolations = errors.filter(e => e.type === 'context_leakage' || e.type === 'context_leakage_suspected').length;
    ...
    generateRecommendations(errors) {
        ...
        if (errorTypes.has('character_name_inconsistency') ||
        ...
        if (errorTypes.has('missing_discriminating_test') ||
