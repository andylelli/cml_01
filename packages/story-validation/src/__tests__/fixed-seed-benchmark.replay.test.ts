...
interface SeedBenchmark {
  run_id: string;
  expected_prose_total: number;
  expected_chapter_count: number;
  expected_chapter_scores: Record<string, number>;
}

interface SeedBenchmarkFixture {
...
describe('Fixed-seed prose benchmarks (chapter-level)', () => {
  ...
  const fixture = readJson(fixturePath) as SeedBenchmarkFixture;
  ...
  it('matches expected prose totals and chapter score signatures for benchmark seeds', () => {
    for (const benchmark of fixture.benchmarks) {
      ...
      expect(
        ...
        `unexpected chapter score count in ${benchmark.run_id}`,
      ).toBe(benchmark.expected_chapter_count);

      const actualScoreMap: Record<string, number> = {};
      ...
      expect(
        actualScoreMap,
        `chapter-level score drift detected in ${benchmark.run_id}`,
      ).toEqual(benchmark.expected_chapter_scores);
    }
  });
});