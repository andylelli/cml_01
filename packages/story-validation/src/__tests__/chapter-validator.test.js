...
describe("ChapterValidator tightening", () => {
    ...
    it("flags scaffold/template leakage prose blocks", () => {
        const leaked = "At The Library in Harrowfield Village, England, the smell of A musty aroma fills the air, " +
...
describe("ChapterValidator — victim identity (chapter 1)", () => {
    ...
    it("flags anonymous victim language in chapter 1", () => {
        ...
        expect(result.issues.some((i) => i.message.includes("anonymous victim") || i.message.includes('"body of an unknown person"'))).toBe(true);
    });
    it("flags chapter 1 that does not name the known victim", () => {
        ...
        const result = validator.validateChapter(chapter, makeCmlWithVictim());
        expect(result.issues.some((i) => i.message.includes("does not name the victim"))).toBe(true);
    });
    it("does NOT flag chapter 1 when the victim's name appears", () => {
        ...
        const result = validator.validateChapter(chapter, makeCmlWithVictim());
        expect(result.issues.some((i) => i.message.includes("does not name the victim") || i.message.includes("anonymous victim"))).toBe(false);
    });
...
const makeCmlWithDiscriminatingTest = () => ({
    CASE: {
        meta: { setting: { location: "Harrowfield Hall" } },
...
//# sourceMappingURL=chapter-validator.test.js.map